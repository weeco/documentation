    ...
    for (var i = 0; i < arr.length; i++) {
        ...
        if (arr[i].endsWith('(')) {
            ...
            if (arr[config_default_value_pos].includes(')')) {
                ...
                while (!arr[config_default_value_pos].endsWith(')')) {
    ...
    for (var j = 0; j < node_arr.length; j++) {
        ...
        if (node_arr[j].endsWith('(')) {
            ...
            if (node_arr[config_type_pos].endsWith('},')) {
                ...
                config_type_pos++;
                while (!node_arr[config_type_pos].endsWith('},')) {
                    config_type_pos++
                }
...
function writeFile(file_name, configs) {
    ...
    let configs_table = '| Property | Description |\n';
    configs_table = configs_table + '|-----|--------------|\n';
        
    let cfg;
    for (var k = 0; k < configs.length; k++) {
        cfg = configs[k];
        configs_table = configs_table + '| __*' + cfg.name + '*__ | ' + cfg.description;

        if (!configs_table.endsWith('.')) {
            configs_table = configs_table + '.'
        }
       
        if (cfg.defaultValue.includes('"')) {
            configs_table = configs_table + '<br/><br/>Default: ' +  cfg.defaultValue.replaceAll('"', '');
        } else if (cfg.defaultValue.includes('{}')) {
            configs_table =  configs_table + '<br/><br/>Default: Null';
        } else {
            configs_table = configs_table + '<br/><br/>Default: ' +  cfg.defaultValue;
        }

        if (cfg.minValue) {
            configs_table = configs_table + '<br/><br/>Minimum = ' + cfg.minValue;
        }

        if (cfg.maxValue) {
            configs_table = configs_table + '<br/>Maximum = ' + cfg.maxValue;
        }

        configs_table = configs_table + ' |\n';
    }

    fs.writeFile(file_name, configs_table, function(err) {
        if (err) {
