...
- EOS helps developers avoid the anomalies of at-most-once processing (with potential lost events) and at-least-once processing (with potential duplicated events). Redpanda supports EOS when transactions are used in combination with [idempotent producers](/docs/development/idempotent-producers). 
...
:::note
Atomicity of transactions is not guaranteed when [remote recovery](../../data-management/tiered-storage/#remote-recovery) is used.
:::
...
- When upgrading, make sure to use maintenance mode with [rolling upgrades](../../install-upgrade/rolling-upgrade/).
:::

The two primary use cases for transactions are:

- [Atomic (all or nothing) publishing of multiple messages](../../development/transactions/#atomic-publishing-of-multiple-messages)
- [Exactly-once stream processing](../../development/transactions/#exactly-once-stream-processing)
