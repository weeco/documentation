...
The `enable_idempotence` property is enabled by default. This property is set at the cluster level, which means the setting applies to all nodes in the cluster. To disable idempotence, set `enable_idempotence` to `false`. For general instructions on how to edit any cluster property, refer to [Configuring cluster properties](/docs/cluster-administration/cluster-property-configuration).