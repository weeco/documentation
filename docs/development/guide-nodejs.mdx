...
<head>
    <meta name="title" content="Node.js Code Example | Redpanda Docs"/>
...
<ThemedImage
  alt="Node.js themed image"
  sources={{
    light: useBaseUrl("/img/nodejs-light.svg"),
    dark: useBaseUrl("/img/nodejs-dark.svg"),
  }}
  width="200"
/>
</div>
...
In this example we will use [kafkajs](https://kafka.js.org/).
...
## Setup a Redpanda environment

Follow our [60-second guides](../../quickstart) to spin up a development
...
This will create a topic named `chat-room`, with the default number of
...
It's just as easy to create topics programmatically too:
...
Now that we have a cluster up and running, and a topic to store our data, let's
...
That's it, a working producer that sends strings entered by the user to the 
`chat-room` topic. We are sending the messages as JSON encoded strings here,
but keep in mind that the producer just sends buffers, so you can encode the
...
There you have it. A consumer that will read all messages from the `chat-room`
topic and print them to the console. You can start as many consumer groups as
you like, but bear in mind that each group will read a message on once. That's
...
Try it yourself, there are endless use cases.
