...
As you probably have heard by now, Redpanda is Kafka® API-compatible, which
means that despite Redpanda being a new streaming data platform, you can 
leverage the countless client libraries that have been created for Kafka®. If 
you find something that is not supported, reach out to our team in the
community <a href="https://redpanda.com/slack">Slack</a>.

In this tutorial we will demonstrate how to get started with a variety of
Kafka® client libraries by creating a topic, producing some data, and consuming
it back.

## Setup Redpanda

Follow our [quickstart](../../quickstart) guide to spin up a development
environment on your platform of choice. If you already have a Redpanda Cloud
cluster, then you're good to go, we'll show you how to connect to that too.
...
Download and install Go from the [go.dev](https://go.dev/doc/install) website.
In this tutorial we will use the [franz-go](https://github.com/twmb/franz-go) 
library.
...
Download and install Node.js from [here](https://nodejs.org/en/download). In 
this tutorial we will use the [KafkaJS](https://kafka.js.org/) library.
...
[python.org](https://www.python.org/downloads). In this tutorial we will use
the [kafka-python](https://kafka-python.readthedocs.io/en/master/) library.
...
Let's get started by creating our first Redpanda topic. You can create a topic
using Redpanda's command line interface 
...
It's just as easy to create a topic programmatically:
...
simple as specifying a list of brokers, but how does this differ in Redpanda
Cloud?

Redpanda Cloud can be configured to use SASL/SCRAM (username and password) or
...
Now that we have a topic, let's create a producer and send some messages:
...
Finally, let's create a consumer to read the data back out of the topic:
...
In this tutorial you have developed the building blocks of a Redpanda client
...
The code examples provided in this tutorial are intentionally simple to help
you get started. For more advanced examples, including how to build stream 
processing applications that can aggregate, join, and filter your data streams,
