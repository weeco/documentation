...
When a topic has cloud storage enabled, you can create a separate remote cluster just for consumers of this topic, and populate its topics from cloud storage. A read-only topic on a remote cluster can serve any consumer, without increasing the load on the original cluster.

You can create Remote Read Replica topics in a Redpanda cluster that directly accesses data stored in cloud object storage. Because these read-only topics access data directly from object storage instead of the topics' original cluster, there's no impact to the performance of the cluster. Furthermore, topic data can be consumed within a region of your choice, regardless of the region where it was produced. If you plan to create a Remote Read Replica topic in another region, consider using a [multi-region bucket](https://aws.amazon.com/s3/features/multi-region-access-points/) to simplify deployment and optimize performance. Sample use cases for Remote Read Replicas include data analytics, offline model training, and development clusters.

## Creating a topic with archival or tiered storage
...
3. Enable cloud storage on the origin cluster by running `rpk cluster config edit`, and then specify the following cluster configuration properties:
    ...
    | `cloud_storage_access_key: “<access_key>”` | The access key used for authentication. When using [IAM roles](https://docs/core/security/iam-roles), you do not have to specify this property. |
...
1. Create a remote cluster for the Remote Read Replica topics. The remote cluster must be in the same region as the S3 bucket associated with the origin cluster. If the S3 bucket associated with the origin cluster is a multi-region bucket, you can create the read replica cluster in any region supported by the bucket.
2. Create an S3 bucket for the remote cluster. The remote cluster cannot share the S3 bucket used for the origin cluster. 
3. Run `rpk cluster config edit` and set the same cluster properties as for the origin cluster. Note that the credentials must allow read access to the S3 bucket of the origin cluster.
4. Create the Remote Read Replica topic by running `rpk topic create <topic_name> -c redpanda.remote.readreplica=<bucket_name>`. For `<topic_name>`, use the same name as the original topic. For `<bucket_name>`, use the S3 bucket specified in the `cloud_storage_bucket` cluster configuration property for the origin cluster.