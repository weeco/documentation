...
ck awareness allows you to distribute replicas of the same partition across different racks to minimize data loss in the event of a rack failure. A rack is a failure zone that has one or more Redpanda <Term popup="An instance of the Redpanda service, which includes the Kafka API, Schema Registry, the Admin API, and the HTTP proxy (PandaProxy)." reference="/docs/terms/broker">brokers</Term> assigned to it.
