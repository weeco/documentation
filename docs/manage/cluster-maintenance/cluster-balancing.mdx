...
1. Set the `partition_autobalancing_mode` property to `off`. If Redpanda partition balancing is enabled, Redpanda may change partition assignments regardless of an `AlterPartitionReassignments` request.
...
3. Put all partition reassignments in a JSON file. For example, to replace node 0 with another node in the replica sets of partitions 1 and 2:
...
4. Execute partition reassignments with the `kafka-reassign-partitions.sh` script. This example uses `example.json` as the name of your JSON file:
...
5. Verify that the reassignment is complete with the flags `--verify --preserve-throttles`: 
  ...
  :::note
  For details on why `--preserve-throttles` is necessary, see the section "Differences between Redpanda and Kafka".
  :::

  Alternatively, run `rpk` again to show verify your reassigned replica sets:
...
### AlterPartitionReassignments in Redpanda

- Kafka comes with a shell script to issue `AlterPartitionReassignments` requests: `kafka-reassign-partitions.sh`. This script attempts to use some throttle configurations that Redpanda does not support, such as `replica.alter.log.dirs.io.max.bytes.per.second`. Redpanda users should include the flag `--preserve-throttles` to avoid errors.

- Kafka supports `AlterPartitionReassignments` requests to upreplicate or dereplicate a topic partition. Redpanda currently doesn't support this.

- An `AlterPartitionReassignments` request lists the IDs for each replica. Kafka validates the node ID for any new replica (that wasn't in the previous replica set) against the list of alive nodes. Redpanda validates all replicas against the list of alive nodes.

- Redpanda supports shard-level partition assignments. Therefore, when resolving an `AlterPartitionReassignments` request, Redpanda automatically determines the shard placements. If you want a topic-partition on a specific shard, you must use the Admin API.

- When there are two identical `AlterPartitionReassignments` requests, Redpanda rejects one of them with the error `no_reassignment_in_progress`.
