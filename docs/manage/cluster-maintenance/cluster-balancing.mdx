...
3. We need to put all partition reassignments in a json file before we issue the request. For example, suppose we want to replace node 0 with another in the replica sets of partitions 1 and 2. The json file would look like:
```json
{
  "version": 1,
  "partitions": [
