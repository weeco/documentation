...
When a topic is created, Redpanda evenly distributes its partitions by sequentially allocating them to the broker with the least number of partitions. By default, Redpanda provides leadership balancing and partition rebalancing when brokers are added or decommissioned. 
...
Automatic leadership balancing improves cluster performance by transferring leadership for a broker's partitions to other replicas. Leadership balancing is enabled by default with the `enable_leader_balancer` property. Automatic leadership balancing changes where data is read to and written to first. It doesn't move any data. For more information, see [Partition leadership elections](../../../get-started/architecture/#partition-leadership-elections).

## Redpanda partition balancing

Partition balancing moves data from hot-loaded brokers to least-loaded brokers to ensure steady-state performance. It ensures predictable and even performance across all brokers in a cluster. Partition balancing is invoked periodically, determined by the `partition_autobalancing_tick_interval_ms` property. Default is 30 seconds.
...
* Monitoring low disk space lets Redpanda distribute partitions across brokers with enough disk space. If free disk space reaches a critically low level, Redpanda blocks clients from producing. For information about the disk space threshold and alert, see [Handle full disks](../disk-utilization/#handle-full-disks).
...
| Setting      | Description |
| ----------- | ----------- |
| `node_add`      | Partition balancing happens when brokers are added.  <br /><br />This is the default setting.|
...
3. Put all partition reassignments in a JSON file. For example, to replace broker 0 with another broker in the replica sets of partitions 1 and 2:
...
1. Verify that the reassignment is complete with the flag `--preserve-throttles`: 
...
- Kafka supports uprelication or dereplication through partition reassignments. Redpanda currently doesn't support this.

- In a partition reassignment, users must provide the broker ID for each replica. Kafka validates the broker ID for any new replica (that wasn't in the previous replica set) against the list of alive brokers. Redpanda validates all replicas against the list of alive brokers.
...
- When there are two identical partition reassignment requests, Redpanda rejects one with the error `no_reassignment_in_progress`.
