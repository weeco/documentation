...
5. We can check if the reassignment is complete with the flags `--verify --preserve-throttles`. For details on why `--preserve-throttles` is necessary, see the section "Differences between Redpanda and Kafka".
```bash
kafka-reassign-partitions --bootstrap-server localhost:9092,localhost:9093,localhost:9094,localhost:9095 --reassignment-json-file example.json --verify --preserve-throttles
