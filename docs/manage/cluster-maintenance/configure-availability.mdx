...
The cluster properties that limit the number of connections:
...
:::note
All these connection limit properties are disabled by default, so they require manual configuration to enable.
:::

:::note
A client typically opens two or three connections, so the total number of connections usually is not equal to the number of clients.
:::
