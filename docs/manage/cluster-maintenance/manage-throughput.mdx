...
- [kafka_quota_balancer_min_shard_throughput_ratio](../../../reference/cluster-properties#kafka_quota_balancer_min_shard_throughput_ratio):	the lowest value of the throughput quota a shard can get in the process of quota balancing, expressed as a ratio of the default shard quota. Zero means there is no minimum, while one means no quota can be taken away by the balancer.
