...
To manage the volume of traffic going through a node, Redpanda implements a throughput quota balancer to enforce node-wide throughput rate limits. The throughput quota accounts for all Kafka API traffic going in and out of a node. The balancer has configurable rate limits of total ingress and egress traffic. To distribute the throughput quota, the balancer periodically monitors the throughput rate of a node's shards, and it distributes quota to the shards that can accept it. The balancer determines a dynamically changing default quota per shard. Each shard has a minimum throughput quota value, which is configurable both as a percentage of the default quota and as an absolute rate limit. 
