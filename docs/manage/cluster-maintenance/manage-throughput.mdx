...
With Redpanda's [thread-per-core model](../../../get-started/architecture#thread-per-core-model), each CPU core (shard) processes the Kafka API traffic of an application thread. When a shard handles Kafka traffic of increasing backpressure, it can improve its performance by throttling down its throughput to relieve the backpressure, while other shards with less backpressure can benefit by handling more traffic.

To manage the volume of traffic going through a node, Redpanda implements a throughput quota balancer to enforce node-wide throughput rate limits. The throughput quota accounts for all Kafka API traffic going in and out of a node. The balancer has configurable rate limits for total ingress and egress traffic. To distribute the throughput quota, the balancer periodically monitors the throughput rate of a node's shards, and it distributes quota to the shards that can accept it. The balancer determines a dynamically changing default quota per shard. Each shard has a minimum throughput quota value, which is configurable both as a percentage of the default quota and as an absolute rate limit. The balancer implements its throttler with a dynamically updated delay that is added to the data path of Kafka API requests. 
...
- By default, both `kafka_throughput_limit_node_in_bps` and `kafka_throughput_limit_node_out_bps` are disabled, so no throughput limits or quota balancing are applied. You must manually set them to turn on quota balancing.
