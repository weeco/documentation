...
To use self-test, first start the test.
...
Before starting, self-test asks for your confirmation to run its large workload.
...
To check on the status of self-test, run its status command:
...
For more details about self-test, including flags, see the rpk cluster self-test command reference.   
