...
Simple Authentication Security Layer (SASL) is a framework that lets you choose the authentication mechanism. The mechanism determines which authentication flow the client uses for that user/password. Redpanda supports the following authentication mechanisms:

- SCRAM (Salted Challenge Response Authentication Mechanism): SCRAM provides strong encryption for user names and passwords by default and does not require an external data store for user information. Redpanda supports  `SCRAM-SHA-256` and `SCRAM-SHA-512`.

- GSSAPI (Kerberos): Redpanda currently supports Kerberos for Bare Metal on-premise customers. 

:::info
Kerberos authentication requires an [Enterprise license](../../../get-started/licenses). To upgrade, contact [Redpanda sales](https://redpanda.com/try-redpanda?section=enterprise-cloud).
:::

When you run an rpk command with SASL/SCRAM, you must include the mechanism with the following flag:
...
To configure SASL authentication for the Kafka API, set the `authentication_method` property of the listener to `sasl`. 
...
2. Ensure that each node has a [Kerberos configuration file](http://web.mit.edu/Kerberos/krb5-latest/doc/admin/conf_files/krb5_conf.html) set to use Active Directory or another corporate key distribution center (KDC). Default is at `/etc/krb5.conf`.

3. Ensure that the KDC has a valid Kerberos service principal name (SPN) for each node in the form `primary/<FQDN>@<REALM>`.

4. Ensure that each node has a keytab containing the SPN for that node. This must be located at an identical file path on each Redpanda node. Default is `/var/lib/redpanda/redpanda.keytab`.
