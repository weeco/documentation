...
| `cloud_storage_segment_max_upload_interval_sec` | The number of seconds for idle timeout. If this property is empty, Redpanda uploads metadata to the cloud storage, but the segment is not uploaded until it reaches the [`segment.bytes`](../../reference/topic-properties#segmentbytes) size. By default, the property is empty. |
