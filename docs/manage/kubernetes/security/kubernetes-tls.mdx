...
By default, a Redpanda cluster communicates internally and externally over a plain-text network connection, where any malicious party can see all communication.
To secure the communication between Redpanda brokers and clients, configure TLS (Transport Layer Security).

To configure TLS you can [use a self-signed certificate](#use-a-self-signed-certificate) for all listeners.
...
1. Enable TLS with self-signed certificates:
...
1. Pass the root CA to rpk to validate your external connection to Redpanda.
  ...
  ```bash
  rpk cluster info --brokers <subdomain>.<custom-domain>:<external-port> --tls-truststore ca.crt
  ```
...
```
kubectl exec redpanda-0 -c redpanda -n redpanda -- rpk cluster info --brokers <subdomain>.<domain>:<external-port>
```
