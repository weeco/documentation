...
<head>
    ...
    <meta name="description" content="Use SASL to provide authentication between Redpanda brokers and clients."/>
</head>

Simple Authentication and Security Layer (SASL) is a method for adding authentication support to connection-based protocols. When using the Redpanda Helm chart, SASL provides authentication between Redpanda brokers and clients.

:::note
You must use [TLS encryption](../kubernetes-tls) to have secure authentication using SASL.
:::
...
:::note
In this YAML file, `auth.sasl.users` is a list of superusers.
:::
