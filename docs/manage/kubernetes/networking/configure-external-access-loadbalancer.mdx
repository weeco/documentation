...
Setting up external access to your Redpanda cluster allows your applications running outside the Kubernetes cluster to communicate with Redpanda and produce and consume events. By default, the addresses brokers advertise are their internal addresses within the Kubernetes cluster, which are not reachable from outside the cluster. To make the cluster accessible from outside, each broker must advertise an address that can be reached from outside the cluster.
...
- If you want brokers to advertise a custom domain, you must have a domain from a domain registrar.
...
By default, the Helm chart gives each broker a subdomain that consists of the chart name and the Pod ordinal. For example, `redpanda-0`, where `redpanda` is the chart name and `0` is the Pod ordinal. To configure brokers to advertise these subdomains, you must configure the brokers with a custom domain.
...
1. [Create a DNS record](#create-dns-records) for each of your default subdomains.

### Advertise custom subdomains

You can give each Redpanda broker a custom subdomain to advertise instead of the default subdomains. Custom subdomains can either be suffixed with the Pod ordinal or they can be fully custom.

#### Advertise fully custom subdomains

1. Add your subdomains to the `external.addresses` setting.

  Replace `<custom-domain>` with your domain, and replace the placeholders in the `external.addresses` setting with your own subdomains in the order that you want them to be applied to the Redpanda brokers. The subdomains must be given to each Redpanda broker in order of the StatefulSet replicas. For example, the Redpanda broker running inside the `redpanda-0` Pod advertises `<subdomain-for-broker-0>.<custom-domain>`.
...
1. [Create a DNS record](#create-dns-records) for each of your subdomains.

#### Advertise custom subdomains suffixed with the Pod ordinal

1. Configure the chart name with the name of your subdomain.
  ...
  This configuration renames your Pods to `<subdomain>-<pod-ordinal>`. Your Redpanda brokers will advertise the `<subdomain>-<pod-ordinal>.<custom-domain>` address.

1. [Create a DNS record](#create-dns-records) for each of your subdomains.

### Create DNS records

When your brokers are configured to advertise a custom domain, the next step is to create DNS records that point the subdomains at the load balancer instances. You can:
...
1. Use your custom domain to communicate with the Redpanda cluster from outside the Kubernetes cluster:
    ...
    ```bash
    rpk cluster info --brokers <subdomain>.<custom-domain>:31092
    ```
...
1. Enable ExternalDNS in the Redpanda Helm chart:
  ...
  When `external.externalDns` is enabled, each LoadBalancer Service is annotated with `external-dns.alpha.kubernetes.io/hostname` and the value is set to the given hostname of each Redpanda broker.
