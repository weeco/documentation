...
To generate a diagnostics bundle, execute the `rpk debug bundle` command inside the container of a Redpanda broker.
...
1. Execute the `rpk debug bundle` command inside a Pod container that's running a Redpanda broker.
  ...
  ```bash
  kubectl exec -it -n redpanda redpanda-0 -c redpanda -- rpk debug bundle --namespace redpanda
  ```
  ...
  ```bash
  kubectl exec -it -n redpanda redpanda-0 -c redpanda -- rpk debug bundle --upload-url <url>
  ```
