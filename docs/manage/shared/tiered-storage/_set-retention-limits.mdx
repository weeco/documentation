...
With Tiered Storage enabled, data expires from local storage following <Link to={props.frontmatter.linkRoot + 'reference/topic-properties#retentionlocaltargetms'}>retention.local.target.ms</Link> or <Link to={props.frontmatter.linkRoot + 'reference/topic-properties#retentionlocaltargetbytes'}>retention.local.target.bytes</Link>. For example, if `retention.local.target.bytes` is set to 10GiB, then the partition has 10GiB/number_of_partitions. (These are equivalent to `retention.ms` and `retention.bytes` without Tiered Storage.)
