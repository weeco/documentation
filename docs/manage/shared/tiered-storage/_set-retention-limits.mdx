...
With Tiered Storage enabled, data in local storage expires after <Link to={props.frontmatter.linkRoot + 'reference/topic-properties#retentionlocaltargetms'}>retention.local.target.ms</Link> or <Link to={props.frontmatter.linkRoot + 'reference/topic-properties#retentionlocaltargetbytes'}>retention.local.target.bytes</Link>. These settings are equivalent to `retention.ms` and `retention.bytes` without Tiered Storage.
