...
For information about the original metrics available on `:9644/metrics`, see [Internal Metrics](../../reference/internal-metrics-reference).
...
| redpanda_uptime_seconds_total | Redpanda uptime in seconds. | none | gauge |
| redpanda_uptime_seconds_total | Total CPU busy time in seconds. | none | gauge |
| redpanda_cloud_client_not_found | Number of requests that came back empty from cloud storage. | Labels differ based on the cloud provider. | --- |
| redpanda_cloud_client_backoff | Number of client requests that backed off. | Labels differ based on the cloud provider. | --- |
| redpanda_cloud_storage_upload_backoff | Number of upload requests that backed off. | Labels differ based on the cloud provider. | --- |
| redpanda_cloud_storage_download_backoff | Number of download requests that backed off. | Labels differ based on the cloud provider. | --- |
| redpanda_cloud_client_uploads | Number of uploads to cloud storage. | Labels differ based on the cloud provider. For S3: endpoint (S3 endpoint) and region (S3 region). For Azure: endpoint (ABS endpoint) and storage_account (ABS account). | counter |
| redpanda_cloud_client_downloads | Number of downloads from cloud storage. | Labels differ based on the cloud provider. For S3: endpoint (S3 endpoint) and region (S3 region). For Azure: endpoint (ABS endpoint) and storage_account (ABS account). | counter |
