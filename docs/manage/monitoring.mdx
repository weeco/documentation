...
Metrics of cluster stability:

- Under-replicated partitions by cluster
- Number of groups for which a node is a leader
- Frequency and total number of leadership transfers
- Partition leadership per broker


## Read/write traffic

When you see abnormal, unhealthy spikes or dips in read/write traffic, look for correlation with the number of connections and logged errors.

Metrics of read/write traffic:

- CPU utilization
- Write traffic
- Read traffic
- Rate of logged errors
- Current number of active connections
- SLA sidecar message traffic


# Latency degradation

Take periodic snapshots and watch out for significant changes over a short duration.

- Producer latency
- Fetch latency

Track the latency of Redpanda's internal inter-node RPCs (raft, etc.) and correlate it with Kafka 
...
- Ratio of used/available capacity storage in cluster (bytes)
...
- Number of online (available) partitions
- Number of offline (unavailable) partitions
- Number of under replicated partitions (low replication factor)
...
Query the aggregated throughput of total bytes of Kafka produce/consume requests for a cluster:
...
A stable cluster has a consistent balance of leaders across all nodes, with a low rate of change in leadership between nodes.

Query the rate of leadership changes across all partitions of a given topic:
