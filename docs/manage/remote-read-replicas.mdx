...
When cloud storage is enabled on a topic, it immediately copies closed log segments into the configured storage bucket. A topic’s object store thus lags behind the local copy by the default [log segment size](../../reference/tunable-properties#log_segment_size).
