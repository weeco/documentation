...
A Remote Read Replica topic is a read-only topic that mirrors a topic on a different cluster. Remote Read Replicas work with both [Tiered Storage](../tiered-storage/) and archival storage.
...
When cloud storage is enabled on a topic, it immediately copies closed log segments into the configured storage bucket. The default log segment size is set to one GiB, so by default, a topic’s object store lags approximately one GiB behind the local copy. To reduce this lag:

- You can lower `segment.bytes`. This lets Redpanda archive smaller log segments more frequently, at the cost of increasing I/O and file count. 
...
1. Create a remote cluster for the Remote Read Replica topic in the same region as the bucket used for the origin cluster. 
...
4. Create the Remote Read Replica topic by running `rpk topic create <topic_name> -c redpanda.remote.readreplica=<bucket_name>`. 
