...
:::caution
In order to prevent any potential data loss when Docker is locally executed, you will need to utilize persistent volumes. You will find the steps to do that here: [persistent volumes](/docs/quickstart/quick-start-docker#create-network-and-persistent-volumes).
:::
...
Finally, you'll need to follow the steps in the [docker getting started](/docs/quickstart/quick-start-docker) to start Redpanda.
...
If you would like to perform a rolling upgrade on your cluster using maintenance mode, refer to [Node Maintenance Mode](/docs/cluster-administration/node-management).
...
Currently, it's only possible to run Redpanda on Docker. Therefore, the [Docker guide](/docs/quickstart/quick-start-docker) would be applicable here.
...
You'll also want to set up a real-time dashboard to monitor your cluster health. That can be  done by following the steps in our [monitoring guide](/docs/cluster-administration/monitoring).
