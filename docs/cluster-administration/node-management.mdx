...
To place a node into maintenance mode, run the following command:
...
To remove a node from maintenance mode (and thus enable the node to start taking leadership of partitions), run the following command:

```bash
rpk cluster maintenance disable &lt;node-id> 
```

To see the maintenance status of nodes in the cluster, run the following command:
