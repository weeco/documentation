...
When a node is placed in maintenance mode, it relinquishes leadership of any partitions, reassigns that leadership to other nodes in the cluster, and ensures that the node cannot gain partition leadership until the node is returned to the cluster.
