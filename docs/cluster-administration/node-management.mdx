...
4. Validate the health of the cluster by running `rpk cluster health` and use external metrics, such as consumer lag. If you encounter any issues, take the node out of maintenance mode by running `rpk maintenance mode disable <node-id>`.
