...
See [Node configuration properties](/docs/reference/node-properties) for a list of node properties, their descriptions, and their default values.
...
To see a sample template that includes all node properties as they appear in the default `redpanda.yaml` file, refer to [Node configuration template](/docs/reference/node-configuration-sample). This template is annotated with descriptions of each property.
