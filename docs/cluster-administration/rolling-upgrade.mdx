...
4. Validate the health of the cluster by running `rpk cluster health`. If the cluster is healthy, the output shows `Healthy: true`. You can also evaluate [external metrics](https://docs/core/cluster-administration/monitoring). If you encounter any issues, take the node out of maintenance mode by running `rpk maintenance mode disable <node-id>`.
