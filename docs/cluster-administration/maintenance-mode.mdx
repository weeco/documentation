...
from noah about the output of this command: 
the PR is still in review, the the simplest form of it right now is yes/no. we’ll also report
- partitions without leaders
- nodes that are down
- if there is no controller node

After you have verified that your cluster is in good health, you can put the node into maintenance mode with the following command:
...
If the process stalls or takes longer than a few minutes, revert the process with the following command: 
...
When you are ready to put the node back online, run the following command: 

```bash
rpk cluster maintenance disable <node_id> 
```

To verify that the node is running and the cluster is healthy, run this command: 
