...
10. Open the **Zones** menu and select the availability zone(s) for your region.
    ...
    If Single AZ was selected in the previous step then select a single zone. If Multi AZ was selected in the previous step then select three availability zones to provide two backups in case one availability zone goes down.
11. Click **Next**.
...
### Prepare deployment environment (prerequisites)
...
* Setup the gcloud [application-default](https://cloud.google.com/sdk/gcloud/reference/auth/application-default/login) credentials.
    ...
    To temporarily use your user credentials for API access run: `gcloud auth application-default login`. This command will open Google's sign in page in a browser window.
...
* Navigate back to the cluster and the wizard will take you back to the deploy agent page.

    Paste the **Client ID** and **Client Secret** in to the client credentials boxes.
