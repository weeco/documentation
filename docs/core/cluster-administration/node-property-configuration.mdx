...
<head>
    <meta name="title" content="Configuring node properties | Redpanda Docs"/>
...
See [Node configuration properties](/docs/core/reference/node-properties) for a list of node properties, their descriptions, and their default values.
...
To see a sample template that includes all node properties as they appear in the default `redpanda.yaml` file, refer to [Node configuration template](/docs/core/reference/node-configuration-sample). This template is annotated with descriptions of each property.
