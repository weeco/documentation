...
must be configured to advertise the location where it, so other brokers in
the cluster can be found. If not, clients connecting to brokers outside
their local network experience connectivity issues.

To initally try out Redpanda, see the Getting Started Guides for
...
If Redpanda is running on the subnet `192.168.4.0/24` with IP address `192.168.4.1`, and the clients are running on the subnet `192.168.5.0/24`:
...
Sometimes its useful to have multiple configurations for accessing Redpanda with multiple routes.
