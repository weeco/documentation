...
For information about the original metrics available on `:9644/metrics`, see [Internal Metrics](/docs/core/reference/monitoring-vectorized).
...
count(count by (topic,partition) 
 (redpanda_cluster_partitions{namespace="kafka"}))
```
