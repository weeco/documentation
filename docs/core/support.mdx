...
<head>
    <meta name="title" content="Support | Redpanda Docs"/>
    <meta name="description" content="Redpanda support guideline."/>
</head>
...
[the manual installation section](/docs/core/deployment/production-deployment#manual-installation)
...
[monitoring docs](/docs/core/cluster-administration/monitoring)
...
[Quick Start Linux Guide](/docs/core/quickstart/quick-start-linux)
...
If you are [Setting up a 3-node cluster](/docs/core/quickstart/quick-start-docker#set-up-a-3-node-cluster) and create a topic with two replicas like the following, then the following command would fail.
...
:::note
The following error message is available since 22.1.6
:::

```bash
TOPIC  STATUS
test   INVALID_REPLICATION_FACTOR: Replication factor must be odd.
```

This happens because `rpk` is using [Raft Consensus Algorithm](https://raft.github.io/#:~:text=What%20is%20Raft%3F,pieces%20needed%20for%20practical%20systems) and an even number is meaningless to it. To fix this error, create a cluster with an odd replication factor. For example:
