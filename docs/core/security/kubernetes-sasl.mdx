...
<head>
    <meta name="title" content="Configuring SASL on Kubernetes | Redpanda Docs"/>
    <meta name="description" content="Alone, SASL authenticates the server and the client. To encrypt communication between the server and the client, combine SASL authentication with TLS encryption."/>
</head>
...
For general security information, see [Redpanda security on Kubernetes](/docs/core/security/security-kubernetes).
...
If you haven’t completed these tasks, you can use one of the following quickstarts to get set up before you configure TLS. Just stop before you install and connect to the Redpanda cluster.
...
The Redpanda GitHub repository contains a `sasl.yaml` sample configuration file you can use to enable SASL authentication: [redpanda-examples/docs/core/example-config/kubernetes/sasl.yaml](https://github.com/redpanda-data/redpanda-examples/blob/main/docs/example-config/kubernetes/sasl.yaml).
...
[redpanda-examples/docs/core/example-config/kubernetes](https://github.com/redpanda-data/redpanda-examples/tree/main/docs/example-config/kubernetes).
:::

### SASL with TLS encryption

Optionally, you can use SASL authentication with [TLS encryption](/docs/core/security/tls-kubernetes) for the Kafka API. To do that, enable TLS for the Kafka API by adding the highlighted lines below to the `kafkaApi` property in the configuration file: 
...
Now that you have your superuser and additional users that can interact with the cluster, use the [rpk reference](/docs/core/reference/rpk-commands) documentation to experiment with the `rpk` commands and create additional users and ACLs. 
