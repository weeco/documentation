...
This document provides details on how to enable SASL authentication on Kubernetes. Alone, SASL authenticates the server and the client. To encrypt communication between the server and the client, combine SASL authentication with <a href="https://docs.redpanda.com/docs/core/security/tls-kubernetes/" target="_self">TLS encryption</a>.

Redpanda supports the SASL/SCRAM authentication method for the Kafka API. For information about Redpanda authentication on Kubernetes in general, see the <a href="https://docs.redpanda.com/docs/core/security/kubernetes-security" target="_self">Redpanda security on Kubernetes</a> document. You will find information there about supported authentication methods for each API. 
...
[redpanda-examples/docs/core/example-config/kubernetes/sasl.yaml](https://github.com/redpanda-data/redpanda-examples/blob/main/docs/core/example-config/kubernetes/sasl.yaml)
...
[redpanda-examples/docs/core/example-config/kubernetes](https://github.com/redpanda-data/redpanda-examples/tree/main/docs/core/example-config/kubernetes)
:::

### SASL with TLS encryption

Optionally, you can use SASL authentication with <a href="https://docs.redpanda.com/docs/core/security/tls-kubernetes/" target="_self">TLS encryption</a> for the Kafka API. To do that, enable TLS for the Kafka API by adding the highlighted lines below to the `kafkaApi` property in the configuration file: 
...
```bash
kubectl apply -f https://raw.githubusercontent.com/redpanda-data/redpanda-examples/main/docs/core/example-config/kubernetes/sasl.yaml
```
...
The superuser can grant permissions to additional users through access control lists (ACLs). For details on how ACLs function in Redpanda, see the [rpk acl reference](/docs/core/reference/rpk-commands/#rpk-acl) documentation. 
...
Now that you have your superuser and additional users that can interact with the cluster, you can use the [rpk reference](/docs/core/reference/rpk-commands) documentation to experiment with the `rpk` commands and create additional users and ACLs. 
...
Use the [rpk reference](/docs/core/reference/rpk-commands) documentation to experiment with the `rpk` commands and when you're ready to clean up the cluster from the brontosaurus tutorial, run this command: 

```bash
kubectl delete -f https://raw.githubusercontent.com/redpanda-data/redpanda-examples/main/docs/core/example-config/kubernetes/sasl.yaml
