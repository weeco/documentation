...
The Redpanda GitHub repository contains a `sasl.yaml` sample configuration file you can use to enable SASL authentication: [redpanda-examples/docs/core/example-config/kubernetes/sasl.yaml](https://github.com/redpanda-data/redpanda-examples/blob/main/docs/core/example-config/kubernetes/sasl.yaml).
...
The superuser can grant permissions to additional users through access control lists (ACLs). For details on how ACLs function in Redpanda, see the [rpk acl](/docs/core/reference/rpk-commands/#rpk-acl) documentation. 
