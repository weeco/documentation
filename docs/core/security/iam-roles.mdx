...
Perform the following steps in your cloud provider console to create the IAM role that will be used to access the S3 storage bucket. Before you can configure IAM roles in Redpanda, you must create the IAM role that will be used to access the S3 storage bucket. The following steps use Amazon Web Services (AWS) terminology, but they apply to Google Cloud Platform (GCP) as well:
