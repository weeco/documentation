...
<head>
    <meta name="title" content="Configuring mTLS on Kubernetes | Redpanda Docs"/>
    <meta name="description" content="mTLS authenticates the server and encrypts communication between the server and client, as one-way TLS does, and also authenticates the client."/>
</head>
...
If you haven’t completed these tasks, you can use one of the following quickstarts to get set up before you configure mTLS. Just stop before you install and connect to the Redpanda cluster: 
...
The Redpanda GitHub repository contains an `mtls.yaml` sample configuration file you can use to enable mTLS authentication: [redpanda-examples/docs/core/example-config/kubernetes/mtls.yaml](https://github.com/redpanda-data/redpanda-examples/blob/main/docs/example-config/kubernetes/mtls.yaml).
...
The table below lists the supported listener configurations for each API with mTLS enabled. See [Listeners](/docs/core/security/security-kubernetes#listeners).
...
For information about external connectivity, including subdomains, see [External Connectivity](/docs/core/security/security-kubernetes#external-connectivity).
...
For information about how certificates are created and used in Redpanda, see [Certificates](/docs/core/security/security-kubernetes#certificates).
...
You can use [rpk commands](/docs/core/reference/rpk-commands) to start producing and consuming to your cluster.
