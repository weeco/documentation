...
:::info
This feature requires an [Enterprise license](/docs/core/introduction/licenses). To upgrade, contact [Redpanda sales](https://redpanda.com/try-redpanda).
:::
...
3. Enable cloud storage on the origin cluster by running `rpk cluster config edit`, and then specify the following cluster configuration properties:
    ...
    | `cloud_storage_access_key: “<access_key>”` | The access key used for authentication. When using [IAM roles](/docs/core/security/iam-roles), you do not have to specify this property. |
