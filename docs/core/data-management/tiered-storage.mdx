...
:::caution
Do not use Tiered Storage with <a href="https://docs.redpanda.com/docs/core/data-management/data-transform/" target="_self">Data Transforms</a>, which is in technical preview. Technical preview features are not supported for production usage.
:::
...
5. The sections below give detailed information on cloud storage configuration options, but the properties listed here are the minimum required properties that you must configure at the cluster level to use Tiered Storage for Amazon S3. Use the `rpk cluster config edit` command to edit these properties:
...
5. The sections below give detailed information on cloud storage configuration options, but the properties listed here are the minimum required properties to use Tiered Storage for Google Cloud Storage. Use the `rpk cluster config edit` command to edit these properties:
