...
This feature is in technical preview and is not supported for production. For more information about technical previews, see [Technical Preview](/docs/core/introduction/tech-preview).
...
Redpanda allows you to deploy custom Node.js programs to handle the entire consume-transform-produce cycle for you. This is handled by a transformation engine built on top of [Wasm](https://developer.mozilla.org/en-US/docs/core/WebAssembly) technology.
...
To enable the coprocessor that runs the data transformation, you must set the configuration properties at the cluster level with (centralized configuration)[/docs/core/cluster-administration/cluster-property-configuration] and enable the engine in the `redpanda.yaml` configuration file.

On the cluster level, you need to set the `enable_coproc` property to `true` using (cluster configuration `rpk` commands)[/docs/core/cluster-administration/cluster-property-configuration]:
```bash
...
You can also refer to [rpk's reference page](/docs/core/reference/rpk-commands) for further commands.
...
- Redpanda Wasm Engine Architecture [article](https://redpanda.com/blog/wasm-architecture/)
- How Redpanda built its data transformation engine with the Wasm runtime [article](https://redpanda.com/blog/data-transformation-engine-with-wasm-runtime/)
