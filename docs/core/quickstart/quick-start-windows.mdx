...
`rpk` is essentially a CLI tool that you can use to run [management and data commands](/docs/core/reference/rpk-commands) on the cluster.
...
- Use the [Quick Start Docker Guide](/docs/core/quickstart/quick-start-docker) to try out Redpanda using Docker.
