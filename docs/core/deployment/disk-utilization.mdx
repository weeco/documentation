...
The `storage_min_free_bytes` configuration field determines the threshold of free space for this write rejection. It works with the following configuration properties, which can be set on any data disk (one drive per node):
