...
Redpanda blocks clients from producing when free disk space reaches a critically low level. This gives you time to take action. In addition, you can monitor your disk space, and you can configure retention and storage properties to ensure the production stability of the cluster. 
...
:::caution
When a Redpanda node runs out of disk space, it terminates. This causes necessary leader reassignment and then replication and rebalancing, impacting performance. If more nodes in the cluster fill up and terminate, traffic concentrates in fewer nodes, also impacting performance. You can use [Continuous Cluster Balancing](/docs/core/cluster-administration/continuous-cluster-balancing) to distribute partitions across nodes with enough disk space.
:::
...
Storage space metrics are refreshed as part of the health monitor’s backend tick processing, which collects node health reports for all members of the cluster. This lets any node see the `storage_space_alert` value for all nodes in the cluster, updated at a frequency determined by the health monitor. 

## Log cleanup and message retention

Retention policies control how long messages (also known as events) are kept on disk before they're deleted or compacted. Use message retention policies to keep messages as long as you need without running out of disk space. 
...
Based on these properties, Redpanda automatically runs a log cleanup process in the background. If you start to run out of disk space, adjust your retention properties to reduce the amount of disk space used.
...
You can create a ballast file to act as a buffer against an out-of-disk outage. A ballast file is an empty file that takes up disk space. If Redpanda runs out of disk space and becomes unavailable, you can delete the ballast file, which clears up some disk space and gives you time to delete topics or records and change your retention properties. You can delete the ballast file as a last resort.
