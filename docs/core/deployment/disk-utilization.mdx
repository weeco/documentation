...
If you exceed your low disk space threshold, Redpanda blocks clients from producing. This gives you time to take action. In addition to this, you can monitor your disk space, and you can configure retention and storage settings to ensure the production stability of the cluster. 
...
Redpanda blocks clients from producing when free disk space is at a critically low level. In that state, Redpanda returns errors to external writers, but it still allows internal write traffic, such as replication and rebalancing. 

The `storage_min_free_bytes` configuration field determines the threshold, or "hard limit", of free space for this write rejection. 

You can also set soft limits for a minimum free disk space _alert_. This soft limit causes error messages to be logged, and affects the value of the `vectorized_storage_disk_free_space_alert` metric, which you can monitor for early warning. The soft limit alert feature works with the following configuration properties, which you can set on any data disk (one drive per node):
...
When either the byte or percent threshold is exceeded, the metric `vectorized_storage_disk_free_space_alert` changes value, and a storage space alert error message is written to the redpanda service log. Note that the alert threshold can be set in bytes or percentage of total space. To disable one threshold in favor of the other, set it to zero. 
