...
Redpanda's Full Disk Handling metrics send warnings about low disk space. If you exceed the low disk space threshold, Redpanda blocks writes to the local node from Kafka producers, giving you time to take action. You can also configure retention and storage settings to ensure the production stability of the cluster. 

:::caution
When a Redpanda node runs out of disk space, it terminates. If more nodes in the cluster fill up and terminate, traffic concentrates in fewer nodes, potentially impacting performance. You can use [Continuous Data Balancing](/docs/core/cluster-administration/data-balancing) to automatically balance disk usage. 
:::

## Full Disk Handling

Redpanda blocks clients from producing when unreserved free disk space is at a critically low level. In that state, Redpanda returns errors to external writers, but it still allows internal write traffic, such as replication and rebalancing. 

The `storage_min_free_bytes` configuration field determines the threshold of free space for this write rejection. It works with the following configuration properties, which you can set on any data disk (one drive per node):
...
When either the byte or percent threshold is exceeded, `vectorized_storage_disk_free_space_alert` issues a storage space alert error message. To disable one threshold in favor of the other, set it to zero. 
...
Disk space monitoring is available in cluster health reports. The `local_monitor` class checks disk space and updates the value of that node’s counters for available and free disk space. It also updates the value of its storage_space_alert field based on any threshold passed. This field is exposed with the `vectorized_storage_disk_free_space_alert` metric:
* 0 = no alert
...
* `retention.ms` - Topic-level setting that specifies how long a message stays on disk before it's deleted. This overrides the `delete_retention_ms` for the topic. 
  ...
  To minimize the likelihood of out-of-disk outages, set `retention.ms` to `86400000`, which equates to one day. There is no default value, but if `retention.ms` is not set at the topic-level, the topic uses the `delete_retention_ms` value. 
...
* `ballast_file_size` - You can increase the ballast file size if it is a very high throughput cluster, or you can decrease the ballast file size if you have very little storage space. The ballast file should be large enough to give you enough time to delete data and configure retention settings if Redpanda crashes, but small enough that you don't waste disk space. In general, set ballast_file_size to approximately 10 times the size of the largest segment to have enough space to compact that topic. Default is `1GiB`. 