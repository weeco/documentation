...
The refreshing of storage space metrics happens as part of the health monitor’s backend tick processing, which collects node health reports for all members of the cluster. This lets any node see the storage_space_alert value for all nodes in the cluster, updated at a frequency determined by the health monitor. 
