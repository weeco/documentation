...
Redpanda provides Full Disk Handling metrics to warn about low disk space, and it blocks writes to the local node from Kafka producers when it passes a low disk space threshold. This gives you time to take action. You could also configure retention and storage settings to ensure the production stability of the cluster. 

:::caution
When a Redpanda node runs out of disk space, it terminates. If more nodes in the cluster fill up and terminate, traffic concentrates in fewer nodes, potentially impacting performance. You can use Continuous Data Balancing to automatically balance disk usage.
:::
