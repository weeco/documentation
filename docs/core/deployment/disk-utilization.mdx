...
If you do run out of disk space, look at metrics for your topics, and adjust your retention settings to reduce the amount of disk space used. Or, add another disk to force a rebalance.
...
* `retention.ms` - Topic-level setting that specifies how long a message stays on disk before it's deleted. 
...
* `retention.bytes` - Topic-level setting that specifies the maximum size of a partition. There is no default. 
...
* `ballast_file_size` - Increase the ballast file size if it is a very high-throughput cluster. Decrease the ballast file size if you have very little storage space. The ballast file should be large enough to give you enough time to delete data and configure retention settings if Redpanda crashes, but small enough that you don't waste disk space. In general, set this to approximately 10 times the size of the largest segment, to have enough space to compact that topic. Default is `1GiB`. 