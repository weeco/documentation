...
When a disk exceeds either the byte or percent threshold, `redpanda_storage_disk_free_space_alert` updates, and a storage space alert error message is written to the Redpanda service log. Note that the alert threshold can be set in bytes or percentage of total space. To disable one threshold in favor of the other, set it to zero. 
