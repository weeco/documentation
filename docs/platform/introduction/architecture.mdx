...
Redpanda is packaged as a single binary. It does not rely on any external systems. It’s easy to install, configure, and operate in any production environment. Redpanda is fully compatible with the Apache Kafka© API, so it works with the ecosystem of tools and integrations that have been built on the Kafka protocol. It can be deployed in your data center or in the cloud on bare metal, containers, and virtual machines.
