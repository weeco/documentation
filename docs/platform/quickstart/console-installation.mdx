---
title: Install Redpanda Console
---

<head>
    <meta name="title" content="Install Redpanda Console | Redpanda Docs"/>
...
The easiest way to get started with Redpanda Console on a local machine is to start Redpanda platform
with Docker compose. Refer to the [Docker Compose page](../console/reference/docker-compose.mdx) to
launch Redpanda platform locally.

### Connect to a remote cluster

To connect Redpanda Console to a Kafka cluster that does not run on the same machine where you want
to run Console, use this command:
...
:::note
The full list of configurations can be found in the [Console reference config](../console/reference/config.mdx).
:::

### Connect to a local cluster

To connect Redpanda Console to a Kafka cluster that is running on your local machine
or in another Docker container run:
...
Take note that only the initial connection is made through your provided `KAFKA_RBOKERS` address.
After the initial connection is established, your client application will talk to the cluster to
get all broker' addresses so that it can communicate to all brokers directly. Each broker returns
the configured `advertised_listener` address that matches the port the initial connection has been
made to. Make sure that this address is reachable by your client application.

:::note
The host networking driver only works on Linux hosts, and is not supported on Docker Desktop for Mac, 
Docker Desktop for Windows, or Docker EE for Windows Server. An alternative way is to use the right
binary for your OS and run this binary instead. We provide [binaries](#binaries-and-linux-packages) 
