...
* Use [Continuous Data Balancing](../../cluster-administration/continuous-data-balancing) to automatically distribute partitions across nodes for more balanced disk usage.
...
:::caution 
Do not set `delete_retention_ms` to `-1` unless you're using [remote write with Tiered Storage](../../data-management/tiered-storage#remote-write) to upload segments to remote storage. Setting it to `-1` configures indefinite retention, which can fill disk space. 
:::
...
For details about how to modify cluster configuration properties, see [Cluster configuration](../../cluster-administration/cluster-property-configuration). 
