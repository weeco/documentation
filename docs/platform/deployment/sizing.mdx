...
Before running performance benchmark testing, set Redpanda into production mode and run the auto-tuning tool (`rpk redpanda tune all`) on every node. This enables the necessary hardware optimizations and ensures that the kernel parameters are set correctly.
