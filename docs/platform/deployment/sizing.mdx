...
As specified by the Kafka API, data retention in Redpanda is based on partition size (`retention.bytes`) or age (`retention.ms`), whichever comes first. These settings create a sliding window of opportunity for consumers to read data from Redpanda before the retention process kicks in and older data is unavailable.

The settings are enforced at the partition level. Each partition writes to its own data directory, into one or more log segments. The size of each log segment is determined by `segment.bytes` (default = one GB). As data is produced on a partition, it is appended to the open log segment. When that log segment reaches `segment.bytes` in size, it is closed, and a new log segment is opened.
