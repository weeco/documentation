...
Redpanda Console searches for all files with the .proto extension, up to a directory depth of five levels. If you want to
use multiple different import paths, you must set the `importPaths` property. A common usecase for this are repositories which use
third-party Protobuf types (for example Google's well-known types) that live in a different directory than your own types.
