...
Redpanda Console searches for all files with the `.proto` extension, up to a directory depth of five levels. To
use multiple different import paths, set the `importPaths` property. For example, if your repository includes
third-party Protobuf types, such as Google's types, that are in a different directory to your own types, set the `importPaths` property to both directories.
