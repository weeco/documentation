...
<head>
  ...
  />
  <meta name="description" content="Reference of tunable cluster properties." />
</head>
...
Enable remote reads for all topics.
...
Enable remote writes for all topics.
...
Maximum number of concurrent readers of remote data per shard (CPU core).  If `null`, the value of [topic_partitions_per_shard](#topic_partitions_per_shard) multiplied by 2 is used.
...
The maximum limit per partition for the number of segments pending deletion from the cloud. Segments can be deleted due to retention or compaction. If this limit is breached and deletion fails, then segments will be orphaned in the cloud and will have to be removed manually.
...
Rate limit for controller access control lists (ACL) and user operations.
...
Upper limit of topic `segment.bytes`. Higher values will be clamped to this limit. 
...
Lower limit of topic `segment.bytes`. Lower values will be clamped to this limit. 
...
Maximum number of partition logs that will be replayed concurrently on startup or flushed concurrently on shutdown.
...
Reserved partition slots on shard (CPU core) 0 on each node.  If this is >= [topic_partitions_per_shard](#topics_partitions_per_shard), no data partitions will be scheduled on shard 0.
