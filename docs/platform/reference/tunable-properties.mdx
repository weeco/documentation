---
title: Tunable Configuration Properties
toc_min_heading_level: 2
toc_max_heading_level: 3
---

<head>
  <meta
    name="title"
    content="Tunable configuration properties | Redpanda Docs"
  />
  <meta name="description" content="Tunable configuration properties reference." />
</head>

Tunable properties are cluster properties that are specifically designed for tuning a cluster.

For information on how to edit tunable properties, refer to [Configuring Cluster Properties](../../cluster-administration/cluster-property-configuration).


## Terminology

This section defines Redpanda terminology used in tunable properties.

### Coprocessor mode

Redpanda's [data transform](../labs/data-transform.mdx) feature, built on top of WebAssembly (Wasm), requires [enabling a coprocessor](../../labs/data-transform/#enable-data-transforms) in order for it to run. 

Properties with `coproc` in their names configure the coprocessor for data transforms.

### PID controller

A proportional-integral-derivative (PID) controller is a common control algorithm. 

Redpanda uses a PID controller for [cloud storage uploads](../../data-management/tiered-storage#upload-controller) and [topic compaction](../../data-management/tiered-storage/#set-retention-limits). Properties with `upload_ctrl` in their names configure the cloud storage upload controller. Properties with `compaction_ctrl` in their names configure the topic compaction controller.


### Leader balancer

When a cluster node crashes or restarts, its leadership of partitions with replicas assigned to it must either be assigned back to the node after it restarts or reassigned to another node. The leader balancer manages the transfer of partition leadership between nodes.

Properties with `leader_balancer` in their names configure the leader balancer.

### Queue depth control (QDC)

Redpanda uses a latency-based queue depth control (QDC) algorithm to monitor and bound producer latency by adjusting the queue depth. 

Properties with `qdc` in their names configure queue depth control.

### Raft




## Properties

<!--
import TOCInline from '@theme/TOCInline';

<TOCInline
  // Only show h3 (properties) headings
  toc={toc.filter((node) => node.level === 3)}
  minHeadingLevel={3}
  maxHeadingLevel={3}
/>
-->

### abort_index_segment_size

Capacity of an abort index segment.

**Units**: number of transactions

**Default**: 50000

**Range**: [1, ...] 

---

### abort_timed_out_transactions_interval_ms

Timer timeout to check for and abort inactive transactions.

**Units**: milliseconds

**Default**: 10000

**Range**: [1, ...] 

---

### alter_topic_cfg_timeout_ms

Time to wait for entries replication in controller log when executing alter configuration request.

**Units**: milliseconds

**Default**: 5000

**Range**: [1, ...] 

---

### append_chunk_size

Size of direct-write operations to disk.

**Units**: bytes

**Default**: 16 KB

**Range**: [4096, 32 MiB]. Must be 4096-byte aligned. 

---

### cloud_storage_cache_check_interval

Timeout to check if cache eviction should be triggered.

**Units**: milliseconds

**Default**: 30000

**Range**: [1, ...]

---

### cloud_storage_enable_remote_read

Enable remote read for all topics.

**Type**: boolean

**Default**: false

---

### cloud_storage_enable_remote_write

Enable remote write for all topics.

**Type**: boolean

**Default**: false

---

### cloud_storage_initial_backoff_ms

Initial backoff time for exponential backoff algorithm.

**Units**: milliseconds

**Default**: 100

**Range**: [1, ...]

---

### cloud_storage_manifest_upload_timeout_ms

Manifest upload timeout.

**Units**: milliseconds

**Default**: 10000

**Range**: [1, ...]

---

### cloud_storage_max_connection_idle_time_ms

Maximum HTTPS connection idle time.

**Units**: milliseconds

**Default**: 5000

**Range**: [1, ...]

---

### cloud_storage_reconciliation_interval_ms

Interval at which the archival service runs reconciliation.

**Units**: milliseconds

**Default**: 1000

**Range**: [1, ...]

---

### cloud_storage_segment_max_upload_interval_sec

Duration that a segment can be kept in local storage without uploading it to remote storage.

**Units**: seconds

**Default**: null (infinite duration)

**Range**: [1, ...] or null (infinite)

---

### cloud_storage_segment_upload_timeout_ms

Log segment upload timeout.

**Units**: milliseconds

**Default**: 30000

**Range**: [1, ...]

---

### cloud_storage_upload_ctrl_d_coeff

Derivative coefficient of the upload PID controller.

**Type**: double

**Default**: 0.0

**Range**: [0.0, ...]

---

### cloud_storage_upload_ctrl_max_shares

Maximum number of IO and CPU shares that the archival upload can use. 

**Units**: number of IO and CPU shares

**Default**: 1000

**Range**: [1, ...]

---

### cloud_storage_upload_ctrl_min_shares 

Minimum number of IO and CPU shares that the archival upload can use.

**Units**: number of IO and CPU shares

**Default**: 100

**Range**: [1, ...]

---

### cloud_storage_upload_ctrl_p_coeff

Proportional coefficient of the upload PID controller.

**Type**: double

**Default**: -2.0

**Range**: (VERIFY)

---

### cloud_storage_upload_ctrl_update_interval_ms

Sampling interval of the upload PID controller.

**Units**: milliseconds

**Default**: 60000

**Range**: [1, ...]

---

### compacted_log_segment_size

Size of a compacted log segment.

**Type**: integer

**Units**: bytes

**Default**: 268435456 (256 MiB)

**Range**: [1 MiB, ...]

---

### compaction_ctrl_backlog_size

Size of the compaction backlog of the backlog controller. If empty (null), defaults to 10% of disk capacity.

**Type**: integer

**Units**: bytes

**Default**: null

**Range**: [1 MiB, ...]

---

### compaction_ctrl_d_coeff

Derivative coefficient for compaction PID controller.

**Type**: double

**Default**: 0.2

**Range**: [0.0, ...]

---

### compaction_ctrl_i_coeff

Integral coefficient for compaction PID controller.

**Type**: double

**Default**: 0.0

**Range**: [0.0, ...]

---

### compaction_ctrl_max_shares

Maximum number of IO and CPU shares that the compaction process can use. 

**Units**: integer

**Default**: 1000

**Range**:

---

### compaction_ctrl_min_shares

Minimum number of IO and CPU shares that the compaction process can use. 

**Type**: integer

**Units**: number of shares

**Default**: 10

**Range**:

---

### compaction_ctrl_p_coeff

Proportional coefficient for compaction PID controller. Must be negative to decrease the compaction backlog when compaction shares increase.

**Type**: double

**Default**: -12.5

**Range**: [..., 0.0)

---

### compaction_ctrl_update_interval_ms

Sampling interval of the compaction PID controller.

**Units**: milliseconds

**Default**: 30000

**Range**: [1, ...]

---

### controller_backend_housekeeping_interval_ms

Duration between iterations of the controller backend's housekeeping loop.

**Units**: milliseconds

**Default**: 1000

**Range**: [1, ...]

---

### coproc_max_batch_size

Maximum size of one topic read in coprocessing mode. 

**Units**: bytes

**Default**: 32 KB

**Range**: [1, ...]

---

### coproc_max_inflight_bytes

Maximum size of inflight data when sending to the Wasm engine.

**Units**: bytes

**Default**: 10485760 (10 MiB)

**Range**: [1, ...]

---

### coproc_max_ingest_bytes

Maximum size of input logs in memory.

**Units**: bytes

**Default**: 655360 (64 KiB)

**Range**: [1, ...]

---

### coproc_offset_flush_interval_ms

Interval for which coprocessor offsets are flushed to disk.

**Units**: milliseconds

**Default**: 300000

**Range**: [1, ...]

---

### create_topic_timeout_ms

Timeout to wait for new topic creation.

**Units**: milliseconds

**Default**: 2000

**Range**: [0, ...]

---

### default_num_windows

Default number of quota tracking windows.

**Units**: number of windows

**Default**: 10

**Range**: [1, ...]

---

### default_window_sec

Default duration of quota tracking window.

**Units**: milliseconds

**Default**: 1000

**Range**: [1, ...]

---

### developer_mode

Flag to enable developer mode. Not recommended for production. 

To print the value of the flag, run rpk with the command: 

```bash
rpk redpanda admin config print
```

**Type**: boolean

**Default**: false

---

### disable_batch_cache

Flag to disable the batch cache in log manager.

**Type**: boolean

**Default**: false

---

### election_timeout_ms

Raft election timeout.

**Units**: milliseconds

**Default**: 1500

**Range**: [1, ...]

---

### enable_pid_file

Enables creation of a process ID lock file. Recommended to be set true. 

**Type**: boolean

**Default**: true

**Range**: [1, ...]

---

### features_auto_enable

Flag to auto-enable new feature flags after upgrades.

**Type**: boolean 

**Default**: true

---

### fetch_reads_debounce_timeout

Duration to wait for the next read in fetch request when minimum number of requested bytes wasn't read.

**Units**: milliseconds

**Default**: 1

**Range**: [1, ...]

---

### fetch_session_eviction_timeout_ms

Minimum time before an unused session will get evicted. Maximum time after which inactive session will be deleted is twice the given configuration value cache.

**Units**: milliseconds

**Default**: 60000

**Range**: [1, ...]

---

### full_raft_configuration_recovery_pattern

Recover raft configurations on start for NTPs that match this pattern.

**Type**: string, or list of strings

**Default**: {}

---

### group_initial_rebalance_delay

A delay at the start of a consumer group rebalancing event to wait for new members.

**Units**: milliseconds

**Default**: 3000

**Range**: [0, ...]

---

### group_new_member_join_timeout

Timeout to wait for new members to join a consumer group.

**Units**: milliseconds

**Default**: 30000

**Range**: [0, ...]

---

### group_topic_partitions

Number of partitions in the internal group membership topic.

**Units**: number of partitions

**Default**: 16

**Range**: [1, ...]

---

### health_manager_tick_interval

Period at which the health manager runs.

**Units**: milliseconds

**Default**: 180000 (3 minutes)

**Range**: [1, ...]

---

### health_monitor_max_metadata_age

Maximum duration that metadata is cached in the health monitor of a non-controller node.

**Units**: milliseconds

**Default**: 10000 (10 seconds)

**Range**: [1, ...]

---

### health_monitor_tick_interval

Period at which the health monitor refreshes cluster state.

**Units**: milliseconds

**Default**: 10000 (10 seconds)

**Range**: [1, ...]

---

### id_allocator_batch_size

Size of one batch message (one log record) of an ID allocator log.

**Type**: integer

**Units**: bytes

**Default**: 1000

**Range**: [1, ...]

---

### id_allocator_log_capacity

Maximum number of messages (batches) of an ID allocator log. Once the log reaches this limit, it is compacted.

**Type**: integer

**Units**: number of log messages (batches)

**Default**: 100

**Range**: [1, ...]

---

### join_retry_timeout_ms

Duration between cluster join retries.

**Units**: milliseconds

**Default**: 5000 (5 seconds)

**Range**: [1, ...]

---

### kafka_max_bytes_per_fetch

Maximum amount of data a consumer receives when fetching a record.

**Type**: integer

**Units**: bytes

**Default**: 67108864 (64 MiB)

**Range**: [1, ...]

---

### kafka_qdc_depth_alpha

Smoothing factor for queue depth control depth tracking.

**Type**: double

**Default**: 0.8

---

### kafka_qdc_depth_update_ms

The update frequency of the queue depth control algorithm.

**Units**: milliseconds

**Default**: 7000 (7 seconds)

**Range**: [1, ...]

---

### kafka_qdc_idle_depth

Queue depth of the queue depth control algorithm when in an idle state.

**Units**: number of queue entries

**Default**: 10

**Range**: [1, ...]

---

### kafka_qdc_latency_alpha

Smoothing parameter for queue depth control algorithm.

**Type**: double

**Default**: 0.002

---

### kafka_qdc_max_depth

Maximum queue depth of the queue depth control algorithm.

**Units**: number of queue entries

**Default**: 100

**Range**: [1, ...], >= [kafka_qdc_min_depth](#kafka_qdc_min_depth)

---

### kafka_qdc_min_depth

Minimum queue depth of the queue depth control algorithm.

**Units**: number of queue entries

**Default**: 1

**Range**: [1, ...], <= [kafka_qdc_max_depth](#kafka_qdc_max_depth)

---

### kafka_qdc_window_count

Number of time windows used by the queue depth control algorithm.

**Units**: number of time windows

**Default**: 12

**Range**: [1, ...]

---

### kafka_qdc_window_size_ms

Duration of a time window used by the queue depth control algorithm.

**Units**: milliseconds

**Default**: 1500

**Range**: [1, ...]

---

### kvstore_flush_interval

Flush interval of the key-value store.

**Units**: milliseconds

**Default**: 10

**Range**: [1, ...]

---

### kvstore_max_segment_size

Maximum size of a segment of the key-value store.

**Type**: integer

**Units**: bytes

**Default**: 16777216 (16 MiB)

**Range**: [1, ...]

---

### leader_balancer_idle_timeout

Timeout at which to run the leader balancer when in an idle state. 

**Units**: milliseconds

**Default**: 120000 (2 minutes)

**Range**: [1, ...]

---

### leader_balancer_mute_timeout

Timeout used to mute groups, where muted groups are removed from consideration as leadership rebalancing targets.

**Units**: milliseconds

**Default**: 300000 (5 minutes)

**Range**: [1, ...]

---

### leader_balancer_node_mute_timeout

Timeout used to mute a node, where a muted node is removed from consideration as leadership rebalancing target.

**Units**: milliseconds

**Default**: 20000 (20 seconds)

**Range**: [1, ...]

---

### log_segment_size

Default size of a log segment, for topics which do not set `segment.bytes`.

**Type**: unsigned 64-bit integer

**Units**: bytes

**Default**: 1073741824 (1 GiB)

**Range**: [1 MiB, ...]

---

### max_compacted_log_segment_size

Max compacted segment size after consolidation.

**Type**: unsigned integer

**Units**: bytes

**Default**: 5368709120 (5 GiB)

**Range**: [1, ...]

---

### max_kafka_throttle_delay_ms

Fail-safe maximum throttle delay on kafka requests.

**Units**: milliseconds 

**Default**: 60000 (1 minute)

**Range**: [1, ...]

---

### members_backend_retry_ms

Duration between members backend reconciliation loop retries.

**Units**: milliseconds

**Default**: 5000 

**Range**: [1, ...]

---

### metadata_dissemination_interval_ms

Interval for metadata dissemination batching.

**Units**: milliseconds

**Default**: 3000 

**Range**: [1, ...]

---

### metadata_dissemination_retries

Number of attempts of looking up a topic's meta data like shard before failing a request.

**Units**: number of retries

**Default**: 30

**Range**: [1, ...]

---

### metadata_dissemination_retry_delay_ms

Delay before retrying a topic-lookup in a shard or other meta-tables.

**Units**: milliseconds

**Default**: 500

**Range**: [1, ...]

---

### metadata_status_wait_timeout_ms

Maximum duration to wait in metadata request for cluster health to be refreshed.

**Units**: milliseconds

**Default**: 2000

**Range**: [1, ...]

---

### metrics_reporter_report_interval

Interval of reports sent to clients from the metrics reporter.

**Units**: milliseconds

**Default**: 86400000 (24 hours)

**Range**: [1, ...]

---

### metrics_reporter_tick_interval

Tick timer interval for capturing metrics. 

**Units**: milliseconds

**Default**: 60000 (1 minute)

**Range**: [1, ...]

---

### node_management_operation_timeout_ms

Timeout for executing node management operations.

**Units**: milliseconds

**Default**: 5000

**Range**: [1, ...]

---

### quota_manager_gc_sec

Period of the quota manager's GC timer.

**Units**: milliseconds

**Default**: 30000 (30 seconds)

**Range**: [1, ...]

---

### raft_heartbeat_disconnect_failures

Number of consecutive failed heartbeat responses before forcibly closing an unresponsive TCP connection. Disabled when set to 0. 

**Type**: unsigned integer

**Units**: number of consecutive failed heartbeat responses

**Default**: 3

**Range**: [0, ...]

---

### raft_heartbeat_interval_ms

Period of raft leader heartbeat.

**Units**: milliseconds

**Default**: 150

**Range**: [1, ...]

---

### raft_heartbeat_timeout_ms

Timeout duration of raft leader heartbeat RPC.

**Units**: milliseconds

**Default**: 3000

**Range**: [1, ...]

---

### raft_io_timeout_ms

Timeout duration for raft I/O.

**Units**: milliseconds

**Default**: 10000 (10 seconds)

**Range**: [1, ...]

---

### raft_max_concurrent_append_requests_per_follower

Maximum number of concurrent requests per follower.

**Type**: unsigned integer

**Units**: number of concurrent requests per follower

**Default**: 16

**Range**: [1, ...]

---

### raft_max_recovery_memory

Maximum amount of memory available for raft follower recovery. If `null`, defaults to 10% of total memory.

**Type**: unsigned integer

**Units**: bytes

**Default**: null

**Range**: [32 MiB, ...]

---

### raft_recovery_default_read_size

Default size of a read issued during raft follower recovery.

**Type**: unsigned integer

**Units**: bytes

**Default**: 524288 (512 KiB)

**Range**: [128, 5 MiB]

---

### raft_replicate_batch_window_size

Maximum size of a batch for raft replication.

**Type**: unsigned integer

**Units**: bytes

**Default**: 1048576 (1 MiB)

**Range**: [1, ...]

---

### raft_smp_max_non_local_requests

Maximum number of non-local requests that can execute concurrently on a CPU core (seastar 'shard'). If `null`, defaults to the value determined by the following formula:

```
topic_partitions_per_shard * (266) * (<number of CPU cores> - 1)
```

For details, see [seastar::smp_service_group_config](https://docs.seastar.io/master/namespaceseastar.html#structseastar_1_1smp__service__group__config).

**Type**: unsigned integer

**Units**: number of requests

**Default**: null

**Range**: [1, ...]

---

### raft_timeout_now_timeout_ms

Timeout of the raft 'timeout now' RPC request.

**Units**: milliseconds

**Default**: 1000

**Range**: [1, ...]

---

### raft_transfer_leader_recovery_timeout_ms

Timeout waiting for follower recovery when transferring leadership.

**Units**: milliseconds

**Default**: 10000 (10 seconds)

**Range**: [1, ...]

---

### readers_cache_eviction_timeout_ms

Duration after which inactive readers will be evicted from the cache of readers.

**Units**: milliseconds

**Default**: 30000 (30 seconds)

**Range**: [1, ...]

---

### reclaim_batch_cache_min_free

Minimum amount of free memory maintained by the batch cache.

**Type**: unsigned integer

**Units**: bytes

**Default**: 67108864 (64 MiB)

**Range**: [1, ...]

---

### reclaim_growth_window

Starting from the last point in time where memory was reclaimed from the batch cache, this is the duration during which the amount of memory to reclaim grows at a significant amount, based on heuristics about the amount of available memory.  

**Units**: milliseconds

**Default**: 3000

**Range**: [1, ...], <= [reclaim_stable_window](#reclaim_stable_window)

---

### reclaim_max_size

Maximum batch cache reclaim size.

**Units**: bytes

**Default**: 4194304 (4 MiB)

**Range**: [1, ...]

---

### reclaim_min_size

Minimum batch cache reclaim size.

**Units**: bytes

**Default**: 131072 (128 KiB)

**Range**: [1, ...]

---

### reclaim_stable_window

If the duration since the last time memory was reclaimed has been longer than this property, the memory usage of the batch cache is considered as stable, so only the minimum size ([reclaim_min_size](#reclaim_min_size)) is set to be reclaimed.  

**Units**: milliseconds

**Default**: 10000 (10 seconds)

**Range**: [1, ...], >= [reclaim_growth_window](#reclaim_growth_window)

---

### recovery_append_timeout_ms

Timeout for raft's append entries RPC.

**Units**: milliseconds

**Default**: 5000

**Range**: [1, ...]

---

### release_cache_on_segment_roll

Flag for whether to release cache when a full segment is rolled.

**Type**: boolean

**Default**: false

---

### replicate_append_timeout_ms

Timeout for raft's append entries RPCs issued while replicating entries.

**Units**: milliseconds

**Default**: 3000

**Range**: [1, ...]

---

### segment_appender_flush_timeout_ms

Maximum duration until buffered data is written.

**Units**: milliseconds

**Default**: 1000

**Range**: [1, ...]

---

### segment_fallocation_step

Size of a file allocation (fallocation) step of a segment.

**Type**: unsigned integer

**Units**: bytes

**Default**: 33554432 (32 MiB)

**Range**: [4096, 1 GiB]. Must be 4096-byte aligned.

---

### storage_read_buffer_size

Size of a read buffer (one per in-flight read, one per log segment).

**Type**: unsigned integer

**Units**: bytes

**Default**: 131072 (128 KiB)

**Range**: [1, ...]

---

### storage_read_readahead_count

Number of read-ahead reads when reading from a segment.

**Type**: signed short

**Units**: number of reads

**Default**: 10

**Range**: [1, ...]

---

### storage_space_alert_free_threshold_bytes

The threshold of free storage (in bytes of free storage) below which a low storage-space alert is logged.

**Type**: unsigned integer

**Units**: bytes of free storage

**Default**: 0

**Range**: [0, ...]

---

### storage_space_alert_free_threshold_percent

The threshold of free storage (in percentage of total storage that is free) below which a low storage-space alert is logged.

**Type**: unsigned integer

**Units**: percentage of total storage that is free

**Default**: 5

**Range**: [0, 50]

---

### topic_fds_per_partition

The number of file descriptors reserved for topics per partition.

**Type**: integer

**Units**: number of file descriptors

**Default**: 5

**Range**: [1, 1000]

---

### topic_memory_per_partition

The amount of memory reserved for topics per partition.

**Type**: unsigned integer

**Units**: bytes

**Default**: 1048576 (1 MiB)

**Range**: [1, 100 MiB]

---

### transaction_coordinator_log_segment_size

Size of the log segment of the Kafka broker's transaction coordinator.

**Type**: unsigned integer

**Units**: bytes

**Default**: 1073741824 (1 GiB)

**Range**: [1, ...]

---

### wait_for_leader_timeout_ms

Timeout to wait for leader election.

**Units**: milliseconds

**Default**: 5000

**Range**: [1, ...]

---

### zstd_decompress_workspace_bytes

Size of the Zstandard (zstd) compression algorithm's memory buffer (workspace) in which a decompressed object is emplaced.

**Type**: unsigned integer

**Units**: bytes

**Default**: 8388608 (8 MiB)

**Range**: [1, ...]

## Suggested reading

- Benchmark of Redpanda and Apache Kafka [article](https://redpanda.com/blog/fast-and-safe/)
- Fast distributed transactions with Redpanda [article](https://redpanda.com/blog/fast-transactions/)
