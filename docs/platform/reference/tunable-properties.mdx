...
### Coprocessor (coproc) mode

Redpanda's [data transform](../labs/data-transform.mdx) feature requires [enabling a coprocessor](../../labs/data-transform/#enable-data-transforms) in order for it to run.  


### PID controller

A proportional-integral-derivative (PID) controller is a common control algorithm. Redpanda uses a PID controller for [cloud storage uploads](../../data-management/tiered-storage#upload-controller) and [topic compaction](../../data-management/tiered-storage/#set-retention-limits).  
