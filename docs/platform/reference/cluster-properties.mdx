...
For information on how to edit cluster properties, see [Configuring Cluster Properties](../../cluster-administration/cluster-property-configuration). Some properties require the cluster to be restarted in order to be applied; see a specific property's reference for whether restart is required.
...
Enable archival storage. Must be enabled `true` in order to use [Tiered Storage](../../data-management/tiered-storage/) or Remote Read Replicas.
