...
| node_id | A number that uniquely identifies the node within the cluster. This property must not be changed after a node joins the cluster.<br/><br/>If unset, Redpanda assigns one automatically. If set, it must be a positive number. <br/><br/>Default: Null |
