---
title: Configuring Producers
---

<head>
    <meta name="title" content="Configuring Producers | Redpanda Docs"/>
...
Along with brokers, topics, consumers, and producers are one of the essential
components of a Redpanda deployment. Within streaming applications, producers
are responsible for sending data to brokers, which are stored in topics made
up of partitions. Producers are client applications that write data to Redpanda
in the form of events. In other woords, the role of the producer is to send
messagesto a topic in the cluster. Producers communicate with Redpanda through
the Kafka Producer API.

When a producer publishes a message to a Redpanda cluster, the leader broker of
the partition being written to receives that message and appends it to the
partition in the target topic, which then sends it to the follower brokers for
replication, making that message immediately durable and resilient to node loss.
Producers can also publish messages to a particular partition.

## Producer configuration options

Redpanda provides a myriad of configuration options for all components of a
...
is not acknowledged by the broker, and is useful when you are working with
Redpanda clusters.
...
This option allows Redpanda to immediately consider a message acknowledged when
it is sent to the broker. This means that Redpanda does not have to wait for any
kind of response from the broker. This is the least safe option because a
single-node crash can cause data loss. However, this setting is useful in cases
where you want to optimize for the highest throughput, and are willing to risk
some data loss.
...
where you can afford some degree of data loss, but prefer gathering as much data
as possible in a given time interval.

#### `acks=1`

The producer waits for an acknowledgment from the leader, but it doesn’t wait
for the leader to get acknowledgments from replicas. This setting doesn’t
...
The producer receives an acknowledgment after the leader and the majority of
(and therefore, implicitly, all) replicas acknowledge the message. This increases
durability at the expense of lower throughput and increased latency.
...
partition and the first fails and is retired, but the second succeeds and the
...
In cases when you configure the producer to be [idempotent](../../idempotent-producers).
...
messages can be compressed easier. The batching configuration can be tuned using:

* `buffer.memory`

    `buffer.memory` is the value that controls the total amount of memory
    available to the producer for buffering. In a case where messages are sent
    faster than they can be delivered to the broker, the producer application
    may run out of memory, which will cause it to either block subsequent send
    calls or even throw an exception. The `max.block.ms` parameter controls the
    amount of time the producer will block before throwing an exception if it
    cannot immediately send messages to the broker.

* `batch.size`

    `batch.size` controls the maximum size of coupled messages that can be
    batched together in one request. The producer will automatically put
    messages being sent to the same partition into one batch. This configuration
    parameter is given in bytes as opposed to the number of messages.

    When the producer is gathering messages to assign to a batch, at some point
    it will hit this byte-size limit, which triggers it to send the batch to
    the broker. However, the producer does not necessarily wait (for as much time
    as set using `linger.ms`) until the batch is full. Sometimes, it can even send
    single-message batches. This means that setting the batch size too large is
    not necessarily undesireable, as it won't cause throttling when sending
    messages; rather, it will only cause increased memory usage.

    Conversely, setting the batch size too small can cause the producer to send
    batches of messages faster, which can cause network overhead, meaning a
    reduced throughput. The default value is usually 16384, but you can set this
    as low as 0, which turns off batching entirely.

* `linger.ms`

    `linger.ms` controls the maximum amount of time the producer will wait before
    sending out a batch of messages, if it is not already full. This means you
    can somewhat force the producer to make sure that batches are being filled
...
* `compression.type`
    ...
    `compression.type` controls how the producer should compress a batch of
    messages before sending it to the broker. The default is `none`, which means
    the batch of messages will not be compressed at all. Compression occurs on
    full batches, so you can improve batching throughput by setting this
    parameter to use one of the available compression algorithms (along wth
    increasing batch size). The available options are: `zstd`, `lz4`, `gzip`,
    and `snappy`.

* Serializers

    Serializers are responsible for converting a message to a byte array. You
    can influence the speed/memory efficiency of your streaming setup by
    choosing one of the built-in serializers or writing a custom one. The
    performance consequences of using serialiers is not typically significant.

    For example, if you opt for the JSON serializer, you will have more data to
    transport with each message because every record will contain its schema in
    a verbose format, which impacts your compression speeds and network
    throughput. Alternatively, going with AVRO or Protobuf allows you to only
    define the schema in one place, while also enabling features like schema
    evolution.
...
Note that there are a wide variety of configuration options within Redpanda. The
...
[Configuring Node Properties](../../cluster-administration/node-property-configuration)
and [Consumer Offsets](../consumer-offsets). 

#### Optimizing for speed

From a producer perspective, when speed is the focus (you want to get data into
...
partition size. You can also test `acks` settings.
...
duplicates, or service downtime, you can test `acks` settings. If you set
`acks=all`, then the producer will wait for all replicas to acknowledge the
message before it can send the next message, resulting in lower latency, because
there is more communication required between brokers. This approach can
guarantee higher durability because the message will be replicated to all brokers.
