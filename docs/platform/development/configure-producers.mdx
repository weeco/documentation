...
in the form of events. In other words, the role of the producer is to send
messages to a topic in the cluster. Producers communicate with Redpanda through
the Kafka API.
...
which partition to produce to, if the key is blank the producer round-robins
between the topic's partitions. If a key is provided, then the partitioner
hashes the key using the murmur2 algorithm and modules across the number of
partitions.
...
* `batch.size`
    ...
    not necessarily undesirable, as it won't cause throttling when sending
...
* `compression.type`
    ...
    parameter to use one of the available compression algorithms (along with
...
`acks=all`, then the producer will wait for a majority of replicas to acknowledge the
