...
## Overriding Helm chart defaults
...
Without a dynamic provisioner, you would need to create the PersistentVolumes manually in order to bind it to corresponding PersistentVolumeClaim.
...
Learn [how to configure storage in the Helm chart](../../manage/configure-storage).
...
:::tip
For greater reliability, Redpanda recommends enabling [rack awareness](../../data-management/kubernetes-rack-awareness) to minimize data loss in the event of a rack failure.
:::
...
See [Networking and Connectivity in Kubernetes](../../manage/networking-and-connectivity)
...
- If you're using a cloud platform, use [IAM roles](../../security/iam-roles) to restrict access to resources in your cluster.

To protect your Redpanda cluster, enable and configure the following security features in the Helm chart:

- [Authentication through SASL](../../security/sasl-kubernetes)
- [Network encryption through TLS](../../security/kubernetes-tls)