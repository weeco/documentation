...
<head>
    ...
    <meta name="description" content="Configure the Helm chart to use emptyDir volumes, hostPath volumes, or PersistentVolumes."/>
</head>
...
- [PersistentVolume](#use-persistentvolumes)
- [hostPath](#use-hostpath-volumes)
- [emptyDir](#use-emptydir-volumes)
...
If you're configuring Redpanda for production, make sure to review the [storage best practices](../../deploy/kubernetes-best-practices#storage).
...
Redpanda recommends using a StorageClass with a dynamic provisioner. See the [best practices](../../deploy/kubernetes-best-practices).
...
Managed Kubernetes platforms and self-managed cloud environments usually provide a dynamic provisioner.
...
Enable [rack awareness](../../data-management/kubernetes-rack-awareness) to minimize data loss in the event of a rack failure.
