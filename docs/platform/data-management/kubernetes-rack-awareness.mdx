---
title: Enabling Rack Awareness in Kubernetes
---

<head>
    <meta name="title" content="Enabling Rack Awareness in Kubernetes | Redpanda Docs"/>
...
To check the value of the `topology.kubernetes.io/zone` key, do the following:
...
```bash
kubectl annotate node kind-worker kind-worker2 kind-worker3 topology.kubernetes.io/zone=rack1
```

In this example, dc1r12c22 is the node name for compute node 22 in rack 12 in data center 1. The annotation is set to dc1r12 so that any Redpanda broker that gets scheduled on that node has its `rack` configuration set to `dc1r12`.
...
1. Make sure that your Node resources include the correct annotations. For example, to check the value of the `topology.kubernetes.io/zone` key, do the following:
