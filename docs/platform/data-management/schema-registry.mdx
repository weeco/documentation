...
Redpanda's schema registry provides the interface to store and manage event schemas. Producers register the schemas of data they send, and consumers retrieve the schemas of data they receive. The registry manages the different versions of schemas as they evolve, so producers and consumers can determine whether the data they exchange is incompatible.

Its API endpoints support the following functionality: 

- Querying supported serialization formats.
- Registering schemas for a subject.
- Retrieving schemas of specific versions.
- Retrieving a list of subjects.
- Retrieving a list of schema versions for a subject.
- Configuring schema compatibilty checking.
...
### Serialization format
 ...
 A data serialization format defines how data is converted to and from the bytes that are stored, published, and received. 
...
2. Install [curl](https://curl.se/) or [Python](https://www.python.org/).
    - If using Python, install the [Requests module](https://requests.readthedocs.io/en/latest/user/install/#install), then create an interactive Python session:
        ...
        ```
3. [Install Redpanda with Docker](/docs/platform/quickstart/quick-start-docker).
...
Registering an evolved schema for an existing subject returns an incremented `id`.
...
If the request is successful, it returns the subject and associated schema:
...
The schema registry API provides DELETE endpoints for deleting a schema or all schemas of a subject: 
- `/subjects/<subject>/versions/<version>`
- `/subjects/<subject>`

A schema can be soft deleted (impermanently) or hard deleted (permanantly), based on the boolean query parameter `permanent`. A soft deleted schema can be retrieved and re-registered. A hard deleted schema cannot be recovered.
...
To list subjects of soft deleted schemas, make a GET request with the `deleted` parameter set true, `/subjects?deleted=true`:
...
To undo a soft deletion, follow the steps first to [retrieve the schema](#retrieve-a-schema-of-a-subject) then to [register the schema](#register-a-schema).


#### Hard delete a schema

:::caution
We don't recommend hard (permanently) deleting schemas in a production system. 

The DELETE APIs are provided primarily for use during the development phase, when schemas are being iterated and revised.
:::
...
If the requests are successful, each request return the version ID of the deleted schema:
...
When you're finished, stop and clean up Redpanda:
