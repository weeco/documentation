...
Redpanda's schema registry provides the interface to store and manage event schemas. Producers register the schemas of data they send, and consumers retrieve the schemas of data they receive. The registry manages the compatibility between different versions of schemas, and it notifies producers and consumers when they exchange data with incompatible schemas.
...
Schemas are stored in the registry as a standard compacted topic. To support updating schemas in the registry, Redpanda uses optimistic concurrency control at the topic level to allow mutating REST calls to any broker. Every broker is symmetric, so there is no need to configure leadership or failover strategies.
