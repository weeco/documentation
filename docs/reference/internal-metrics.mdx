...
The `/metrics` endpoint supports aggregation across labels. To enable aggregation, set the [aggregate_metrics](../../reference/cluster-properties#aggregate_metrics) property.

:::caution
Enabling aggregation with `/metrics` generates a lot of monitoring data. It should be enabled only for development and testing when partition-granularity metrics are needed; it should not be enabled for production systems.
:::

To see all available metrics, query the admin server:

```bash
curl <broker-address>:9644/metrics | grep "[HELP|TYPE]"
```
