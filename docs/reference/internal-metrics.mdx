...
For information about the latest metrics available on `:9644/public_metrics`, see [Monitoring](/docs/cluster-administration/monitoring).
...
Redpanda exports Prometheus metrics on `:9644/public_metrics`. To generate the Redpanda configuration on an existing Prometheus instance for both the `/public_metrics` endpoint as well as the `/metrics` endpoint, run: 
...
* See rpk details in the [rpk command reference](/docs/reference/rpk/rpk-generate/rpk-generate-prometheus-config/) for Prometheus. 
:::
...
See details in the [rpk command reference](/docs/reference/rpk/rpk-generate/rpk-generate-grafana-dashboard/) for Grafana. 
