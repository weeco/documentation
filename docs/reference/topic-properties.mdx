...
<head>
  ...
  <meta
    name="description"
    content="A reference of topic-level Kafka properties for Redpanda that override corresponding node or cluster properties."
  />
</head>
...
A reference of topic-level Apache Kafka properties for Redpanda that override corresponding node or cluster properties.

:::note
The documented names, valid values, and default values of Kafka topic-level properties are from Kafka v3.4. Differences are documented with each property.
:::
...
:::note
While the [`rpk topic describe`](../rpk/rpk-topic/rpk-topic-describe) command doesn't report `replication.factor` as a configuration, `replication.factor` can indeed be set via the [`rpk topic alter-config`](../rpk/rpk-topic/rpk-topic-alter-config) command.
:::
