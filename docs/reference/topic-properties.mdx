...
Many topic-level properties override the value of a corresponding [cluster property](../../manage/cluster-maintenance/cluster-property-configuration). If a topic-level property isn't set, the corresponding cluster property takes effect by default.
...
A list of the cleanup policies to apply for log segments of a topic. The available cleanup policies:
...
The duration that a log segment is active&mdash;being written to by producers and not deletable&mdash;for a topic. It's the periodic timeout at which a log closes the active segment of a topic and transitions, or rolls, to a new active segment. The closed, inactive segment is then eligible to be cleaned up according to retention properties.
...
The maximum size of a log segment for a topic. When the size of an active segment exceeds `segment.bytes`, the segment is closed and a new active segment is created. The closed, inactive segment is then eligible to be cleaned up according to retention properties.
...
The maximum size of a message or batch of a topic. If a compression type is enabled, it sets the maximum size of the compressed message or batch.
...
A flag for enabling Redpanda to upload data for a topic from local storage to object storage. When set `true` together with [redpanda.remote.read](#redpandaremoteread), it enables the [Tiered Storage](../../manage/tiered-storage) feature.
...
A flag for enabling Redpanda to fetch data for a topic from object storage to local storage. When set `true` together with [redpanda.remote.write](#redpandaremotewrite), it enables the [Tiered Storage](../../manage/tiered-storage) feature.
...
A size-based retention limit that configures the maximum size that a topic partition in local storage of Tiered Storage can grow before becoming eligible for cleanup. It is equivalent to [retention.bytes](#retentionbytes) without Tiered Storage.
...
A time-based retention limit that configures the maximum duration that a log's segment file for a topic in local storage of Tiered Storage is retained before it becomes eligible to be cleaned up. It is equivalent to [retention.ms](#retentionms) without Tiered Storage.
