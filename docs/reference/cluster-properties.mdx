...
Default lifetime of log segments for topics which do not set `segment.ms` to override it. If `null`, the property is disabled, and no default lifetime is set. It's also accessible via the Kafka API under its Kafka-compatible alias, `log.roll.ms`.
