...
If set to `0.0`, the minimum is disabled. If set to greater than `1.0 / <number-of-shards>`, the balancer won't need to balance anything, as each shard is assigned an equal minimum and together they sum up to at least `1.0`.
