...
The period at which the intra-node throughput quota balancer runs. If `0`, the balancer is disabled and all throughput quotas are immutable.
...
The minimum value of the throughput quota a shard can get in the process of quota balancing, expressed as a ratio of default shard quota. While the value applies equally to ingress and egress traffic, the default shard quota can be different for ingress and egress and therefore result in different minimum throughput bps values. 

If `0`, the minimum is disabled. If `1`, no quota can be taken away by the balancer.
...
**Range**: [number of available CPU cores, ...]
