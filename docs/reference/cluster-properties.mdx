...
The minimum value of the throughput quota a shard can get in the process of quota balancing, expressed as a ratio of default shard quota. If `0`, the minimum is disabled. If `1`, no quota can be taken away by the balancer.
...
The minimum value of the throughput quota a shard can get in the process of quota balancing, expressed in bytes per second. If `0`, no minimum is enforced.
...
The maximum rate of ingress Kafka traffic for a node. If `null`, the property is disabled. 
...
The maximum rate of egress Kafka traffic for a node. If `null`, the property is disabled. 
