...
If set to `0.0`, the minimum is disabled. If set to greater than `1.0 / <number-of-shards>`, where each shard is allocated more than an equal share of total quota, then the balancer won't be able to rebalance quota without violating this ratio, consequently precluding the balancer from running; this is effectively the same as disabling the balancer.
