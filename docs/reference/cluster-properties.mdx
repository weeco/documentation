...
Enable aggregation of metrics returned by the [/metrics](./internal-metrics-reference) endpoint. Metric aggregation is performed by summing the values of samples by labels and is done when it makes sense by the shard and/or partition labels.
