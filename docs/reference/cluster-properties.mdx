...
Default lifetime of log segments. If `null`, the property is disabled, and no default lifetime is set. It's also accessible using the Kafka API under its Kafka-compatible alias, `log.roll.ms`.

`log_segment_ms` is overridden per topic by the topic property [`segment.ms`](../topic-properties#segmentms).
...
`delete_retention_ms` is overridden per topic by the topic property [`retention.ms`](../topic-properties#retentionms).
...
`log_cleanup_policy` is overridden per topic by the topic property [`cleanup.policy`](../topic-properties#cleanuppolicy).
...
`log_compression_type` is overridden per topic by the topic property [`compression.type`](../topic-properties#compressiontype).
...
`log_message_timestamp_type` is overridden per topic by the topic property [`message.timestamp.type`](../topic-properties#messagetimestamptype).
...
`default_topic_replications` is overridden per topic by the topic property [`replication.factor`](../topic-properties#replicationfactor).
...
`retention_bytes` is overridden per topic by the topic property [`retention.bytes`](../topic-properties#retentionbytes).
...
:::caution
[Redpanda Data Transforms](../../labs/data-transform), built on top of Wasm, is a lab feature and is not supported for production deployments.
:::
