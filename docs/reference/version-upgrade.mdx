...
:::caution
Be advised for Kubernetes locally executed, there could be a loss of the data and topics. To avoid data loss make sure to use persistent volumes.
:::
...
Once the changes are applied, the cluster will perform a rolling upgrade, restarting one pod at a time. During the upgrade, the status field will show as  “upgrading” is true. You will be able to see the status of the cluster by running the following command:
...
Also, you will need to make certain that there is not a Redpanda container up and running:
...
:::caution
Be advised concerning Docker locally executed, there could be a potential loss of the data and topics. In order to avoid any data loss you will need to utilize [persistent volumes](/docs/getting-started/quick-start-docker#create-network-and-persistent-volumes).
:::
...
Finally, you’ll need to follow the steps in the [docker getting started](/docs/getting-started/quick-start-docker) to start Redpanda.
If you are using  a previous `redpanda.yaml` configuration file, make sure to apply it to the configuration folder `/etc/redpanda` inside the container. Once completed, restart the cluster:
...
Currently, it’s only possible to run Redpanda on Docker. Therefore, the [Docker guide](/docs/getting-started/quick-start-docker) would be applicable here.
...
You’ll also want to set up a real-time dashboard to monitor your cluster health; That’s easily done by following the steps in our [monitoring guide](/docs/deploy-self-hosted/monitoring).
