...
`kafka_batch_max_bytes` is overridden per topic by the topic property [`max.message.bytes`](../topic-properties#maxmessagebytes).
...
`log_segment_size` is overridden per topic by the topic property [`segment.bytes`](../topic-properties#segmentbytes).
