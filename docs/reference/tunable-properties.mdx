...
When the duration since the last received heartbeat request is at least this timeout, a node considers itself as isolated.
...
Period at which Redpanda checks for expired group offsets.
...
Retention duration of a consumer group offset. Once elapsed, an offset is considered to be expired. If `null`, the property is disabled.
...
The maximum throughput of Kafka traffic allowed on the ingress side of each node. If `null`, the property is disabled.
...
The maximum throughput of Kafka traffic allowed on the egress side of each node. If `null`, the property is disabled.

**Units**: bytes per second

**Default**: null

**Range**: [number of available CPU cores of the node, ...]
...
The lower bound for the topic `segment.ms` property, where lower values are clamped to this.
...
The upper bound for the topic `segment.ms` property, where higher values are clamped to this.
