...
The maximum throttle delay for Kafka responses. Used as a fail-safe when throttle values for request-responses are not set. Also used by the [throughput quota balancer](../../manage/cluster-maintenance/manage-throughput#node-wide-rate-limits) as the maximum delay when allocating quota.
