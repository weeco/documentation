...
The `ballast_file` tuner improves debuggability and recoverability of a system with a full disk by creating a ballast file: if the disk becomes full, the ballast file can be deleted, thereby allowing the the operating system to boot and the root cause to be troubleshooted.
