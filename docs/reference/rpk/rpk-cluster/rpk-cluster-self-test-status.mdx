...
Returns status of currently running self-test or the last completed test.

Use this command after invoking [rpk cluster self-test start](../../rpk-cluster/rpk-cluster-self-test-start) to determine the status of the jobs launched. 

If one or more jobs are still running, then the command returns the node IDs of Redpanda nodes that are still running self-tests.

If no jobs are running, then the command returns the last completed test results for all nodes. 
...
<table>
  <tbody>
    ...
    <tr>
      <td>--format</td>
      <td>string</td>
      <td>Output format: <code>text</code>, <code>json</code>. Default: <code>text</code>.</td>
    </tr>
    <tr>
