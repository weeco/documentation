...
All `rpk` commands offer auto-completion. You can also run `rpk <command> -h` to see all flags for that command, as well as examples.

This section lists each RPK command in alphabetical order, along with a table of flags for that command. All descriptions are from the output of the `rpk –help` command.
