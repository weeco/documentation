...
Redpanda's command line interface (CLI) utility is known as `rpk` (Redpanda Keeper). These commands let you configure, manage, and tune Redpanda clusters. They also let you manage topics, groups, and access control lists (ACLs).

This section lists the `rpk` commands in alphabetical order. Descriptions come from the output of `rpk <command> -–help` and include flags for that command.
