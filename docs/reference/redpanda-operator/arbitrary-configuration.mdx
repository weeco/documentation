...
The cluster definition includes commonly used cluster and node properties. To add other node properties, you can specify any node property in the [node configuration template](../../reference/node-configuration-sample/) by using the `additionalConfiguration` property.
