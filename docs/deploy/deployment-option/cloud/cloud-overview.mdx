...
Redpanda Cloud employs a control plane and data plane architecture. The control
plane configures and maintains Redpanda Cloud clusters, which are self-contained
and independent of the control plane, and remain available even if the network
connection to the data plane is lost.
...
* [Dedicated Cloud](../create-dedicated-cloud-cluster-aws)
* Bring Your Own Cloud (BYOC) ([AWS](../create-byoc-cluster-aws) or [GCP](../create-byoc-cluster-gcp))
...
necessary IAM policies required to run the agent. For details, refer to
[IAM Policies](../security/cloud-security-architecture/authorization/cloud-authorization#aws-iam-policies).  
