...
2. Select the Snowflake managed connector.
...
### Advanced Snowflake connector configuration
...
- `JSON` formatted records should use `"com.snowflake.kafka.connector.records.SnowflakeJsonConverter"`.

- `AVRO` formatted records that use Kafka’s Schema Registry Service should use `"com.snowflake.kafka.connector.records.SnowflakeAvroConverter"`.

- `AVRO` formatted records that contain the schema (and therefore do not need Kafka’s Schema Registry Service) should use `"com.snowflake.kafka.connector.records.SnowflakeAvroConverterWithoutSchemaRegistry"`.

- Plain text formatted records should use `"org.apache.kafka.connect.storage.StringConverter"`.
...
Snowflake connector attempts to authenticate to the Snowflake database to validate
