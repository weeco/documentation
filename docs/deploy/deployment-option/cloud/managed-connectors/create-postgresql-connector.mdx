...
2. Select the (Import from) PostgreSQL (Debezium) managed connector.
...
| **Error while fetching metadata with correlation id 3163753:{.....=UNKNOWN_TOPIC_OR_PARTITION}** | Desired topic does not exist. The `auto_create_topics_enabled` property is disabled and should not be enabled in production, so the topic must be created manually. Usually the topic matches `topic_prefix.full DB table name`. The `topic.creation.enable` property requires [additional configuration](https://debezium.io/documentation/reference/stable/configuration/topic-auto-create-config.html). |
