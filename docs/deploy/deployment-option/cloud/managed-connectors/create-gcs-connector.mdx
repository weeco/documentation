...
4. Create a [custom role](https://cloud.google.com/iam/docs/creating-custom-roles),
   which must have the following permissions:
    ...
    - `storage.objects.create` and `storage.objects.delete` to access items in the GCS bucket
...
2. Select the Google Cloud Storage managed connector.
3. On the **Create Connector** page, specify the following required connector configuration options:
   ...
   | `GCS file format`              | Format of the key coming from the Kafka topic: `CSV` (the default), `JSON`, `JSONL AVRO`, or `PARQUET`. The default is `CSV`. You can use the  `CSV` format output only with `org.apache.kafka.connect.converters.ByteArrayConverter` and `org.apache.kafka.connect.storage.StringConverter`. |
   ...
   | `Max Tasks`                    | Maximum number of tasks to use for this connector. The default `1`. Each task replicates exclusive set of partitions assigned to it. |
...
| Property                     | Description |
| ---------------------------- | ----------- |
| `File name template` | The template for file names on GCS. Supports `{{ variable }}` placeholders for substituting variables. Current supported variables are: `topic`, `partition`, and `start_offset` (the offset of the first record in the file). Only the following combinations of variables are valid: `topic`, `partition`, and `start_offset`. |
