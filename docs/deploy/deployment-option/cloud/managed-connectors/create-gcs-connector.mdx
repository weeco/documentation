...
3. On the **Create Connector** page, specify the following required connector configuration options:
   ...
   | `GCS file format`              | Format of the files created in GCS: `CSV` (the default), `JSON`, `JSONL AVRO`, or `PARQUET`. The default is `CSV`. You can use the  `CSV` format output only with `org.apache.kafka.connect.converters.ByteArrayConverter` and `org.apache.kafka.connect.storage.StringConverter`. |
