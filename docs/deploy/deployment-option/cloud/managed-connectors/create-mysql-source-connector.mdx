...
- There must be a [binlog enabled](https://debezium.io/documentation/reference/stable/connectors/mysql.html#enable-mysql-binlog) for the source MySQL cluster.
...
2. Select the (Import from) MySQL (Debezium) Source managed connector.
...
Schema history configuration errors are reported as failed tasks.
...
| **ConnectException: Error configuring an instance of KafkaSchemaHistory; check the logs for details** | Verify that the schema history configuration properties `schema.history.internal.kafka.topic` and `schema.history.internal.kafka.bootstrap.servers` are set and have valid values. Also check the logs for more details. |
| **No resolvable bootstrap urls given in bootstrap.servers** | Set valid broker URLs using `schema.history.internal.kafka.bootstrap.servers`. |
...
| **org.apache.kafka.connect.errors.ConnectException: Error configuring an instance of KafkaSchemaHistory; check the logs for details** | Set the database schema history properties `schema.history.internal.kafka.*`. |
| **java.lang.IllegalArgumentException: Only a single connector task may be started** | Remove `tasks.max` or set it to `1`. |
