...
2. Select the Export to MongoDB (Sink) managed connector.
3. On the **Create Connector** page, specify the following required connector configuration options:
   ...
   | `MongoDB Connection URI` | The MongoDB connection URI string to connect to your MongoDB instance or cluster. For example, `mongodb://locahost/`. |
...
| `Type of value projection to use` Use either `AllowList` or `BlockList` | **<---THIS MAKES NO SENSE--NONE appears to be the default, but the description gives two other options. The description is in the property name.** The type of value projection to use: `AllowList` or `BlockList`. The default is `NONE`. |
...
- `STRING` when your messages contain plaintex JSON.
