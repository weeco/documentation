...
When Tiered Storage is enabled on a topic, it immediately copies closed log segments into the configured storage bucket or container. The log segment size is set to 128MiB by default (In version 22.3, it is 1GiB), a topic’s object store lags approximately 128MiB behind the local copy. You can also set an idle timeout to force Redpanda to periodically archive the contents of open log segments to object storage. This is useful if a topic’s write rate is low and log segments are kept open for long periods of time.
