...
See also: [Rack Awareness](../../../../../manage/rack-awareness)
...
Producer acknowledgment defines how producer clients and broker leaders communicate their status while transferring data. The [`acks`](../../../../../develop/produce-data/producer-properties#acks) value determines producer and broker behavior when writing data to the event bus. 

See also [`acks` in Message Reliability](../../../../../develop/produce-data/message-reliability#acks)
...
To make Redpanda aware of the topology it's running on, you need to configure the cluster to enable rack awareness and then configure each broker with the identifier of the rack.
...
With rack awareness enabled, Redpanda ensures that no more than a minority of replicas appear on a single rack, even during cluster balancing.
