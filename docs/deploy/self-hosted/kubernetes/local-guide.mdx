...
In this step, you deploy Redpanda and Redpanda Console. Redpanda Console is included as a subchart in the Redpanda Helm chart.
...
1. Create an alias to simplify the `rpk` commands:
  ...
  ```bash
  alias container-rpk="kubectl -n redpanda exec -ti redpanda-0 -c redpanda -- rpk --brokers redpanda-0.redpanda.redpanda.svc.cluster.local.:9093,redpanda-1.redpanda.redpanda.svc.cluster.local.:9093,redpanda-2.redpanda.redpanda.svc.cluster.local.:9093"
  ```

1. Create a topic called `twitch_chat`:

  ```bash
  container-rpk topic create twitch_chat
  ```
...
1. Describe the topic:
  ...
  ```bash
  container-rpk topic describe twitch_chat
  ```
...
1. Produce a message to the topic:
  ...
  ```bash
  container-rpk topic produce twitch_chat
  ```
...
1. Consume one message from the topic:
  ...
  ```bash
  container-rpk topic consume twitch_chat --num 1
  ```
