...
Before continuing please go over the [Kubernetes Quick Start Guide](/docs/quickstart/kubernetes-qs-cloud/) and follow the steps for setting up the Redpanda Operator and optionally a Redpanda cluster.
...
However, Redpanda offers a broad range of custom properties for [node configuration](/docs/cluster-administration/node-property-configuration) besides cluster configuration.
