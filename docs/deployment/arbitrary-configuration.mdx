...
Follow [60-Second Kubernetes for Cloud](/docs/quickstart/kubernetes-qs-cloud/) to set up the Redpanda operator and optionally a Redpanda cluster.
...
The cluster definition includes commonly used cluster and node properties. To add other node properties, you can specify any node property in the [node configuration template](/docs/reference/node-configuration-sample/) by using the `additionalConfiguration` property.
