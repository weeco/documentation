...
The following table shows the properties that are described in this section along with their scope and defaults. If no value is specified in the topic-level configuration, the topic uses the value of the cluster configuration. Note that properties in the cluster configuration use snake case while topic-level configurations use dots: 
...
A message is deleted if its age exceeds the  `delete_retention_ms`  or `retention.ms` value. The properties are explained in detail below: 

* **delete_retention_ms** - Cluster configuration property that specifies how long a message stays on disk before it is deleted. To set retention time for a single topic, use the `retention.ms` property,  which overrides the `delete_retention_ms` setting. If `retention.ms` is not set at the topic-level, the topic uses the `delete_retention_ms` value. 
  ...
  For information about how to modify cluster configuration properties, see [Cluster configuration](/docs/cluster-administration/cluster-configuration). 

* **retention.ms** - Topic-level setting that specifies how long a message stays on disk before it is deleted. This setting overrides the cluster configuration `delete_retention_ms` property for the topic. To minimize the chance of out-of-disk outages, you can set `retention.ms` to `86400000`, which is one day. There is no default, but if `retention.ms` is not set at the topic-level, the topic uses the `delete_retention_ms` value. 
...
* **retention_bytes** - Cluster configuration property that specifies the maximum size of a partition. When a partition reaches the `retention_bytes` size, the oldest messages in the partition are deleted. To set retention size for a single topic, use the `retention.bytes` property, which overrides the `retention_bytes` setting. If `retention.bytes` is not set at the topic-level, the topic uses the `retention_bytes` value.
  ...
  For details about how to modify cluster configuration properties, refer to [Cluster configuration](/docs/cluster-administration/cluster-configuration). 

* **retention.bytes** - Topic-level setting that specifies the maximum size of a partition. This setting overrides the cluster configuration `retention_bytes` property for the topic. We recommend the same considerations for the `retention.bytes` property as the `retention_bytes` property described above. There is no default, but if `retention.bytes` is not set at the topic-level, the topic uses the `retention_bytes` value. 
...
You can set the log segment size for the cluster in the cluster configuration, but if you know which topics will receive more data, it is better to set the size for each topic. Configure log segment size on a topic with the following `rpk` command:
...
To enable ballast file creation, configure the following properties in the `redpanda.yaml` file for the node:
...
The properties are explained in detail below: 
...
time Redpanda is started. The output properties of `rpk iotune` are written to a file
