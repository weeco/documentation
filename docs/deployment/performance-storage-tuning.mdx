...
Retention refers to the conditions under which messages in a topic are retained on disk. These conditions can be configured based on message age or aggregate message size in a topic. The most common reason Redpanda runs out of disk space is high retention settings.
...
A message is deleted if its age exceeds the  `delete_retention_ms`  or `retention.ms` value. Note that properties in `redpanda.yaml` use camel case while topic-level configurations use dots. The parameters are explained in detail below: 
...
A message is deleted if the size of the partition in which it is contained reaches the `retention_bytes` or `retention.bytes` value. Note that properties in `redpanda.yaml` use camel case while topic-level configurations use dots. The parameters are explained in detail below: 
...
:::note
Although a ballast file is not created by default, `rpk mode production` enables the ballast file tuner, which creates the ballast file when a tune occurs. If you follow the Redpanda production recommendations, a ballast file will already exist in the Redpanda data directory. 
:::
