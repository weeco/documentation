...
:::tip
Use one of the Redpanda Kubernetes quickstart guides to install the Redpanda operator and create a cluster in a few easy steps. We have quickstart guides for [cloud](/docs/quickstart/kubernetes-qs-cloud), [kind](/docs/quickstart/kubernetes-qs-local-access), and [minikube](/docs/quickstart/kubernetes-qs-minikube).
:::
...
Note that the Redpanda operator does not check for version compatibility of the properties in the `additionalConfiguration` section of the CRD. You might try to apply a property that is not supported by the Redpanda version that the cluster is running. Therefore, we recommend that you verify the CRD after you make a change. This is explained in the *Verifying the CRD* section below.

### Modifying the CRD

To modify properties, all you have to do is edit the CRD file, save it, and execute it with the following command:
...
If you want to view whether a change to a property requires a restart, see the [Cluster-level properties](/docs/reference/cluster-properties) documentation.

:::caution
It is possible to change cluster-level configuration properties through the Admin API. However, if the property in the CRD contains a different value than the value that you set through the Admin API, the value set through the Admin API will eventually be overwritten. If the property does not exist in the CRD, you can change it through the Admin API and there will be no conflict. However, because there is the possibility of a property value being overwritten, we recommend that you set properties in the CRD.
:::
...
In the example, there is a typo in the `cluster_id` property name, with an additional `d`. After saving the file with the typo, you run the `kubectl apply` command to apply the modified CRD.

To verify whether the cluster was configured successfully, you can run this command:

```
kubectl get cluster -o yaml
```

If there was an error, the `status` section of the output will notify you. In this example, the status will look something like this:
...
After you fix the typo, the `status` section of the `kubectl get cluster-o yaml` output will look something like this:
...
For this example, I'll change the cluster ID to `mapusaurus`. To do that, I edit the `cluster_id` property in the CRD so that the `additionalConfiguration` section looks like this:
...
I then save the file, and run the following command to apply the change:

`kubectl apply -f one-node-cluster.yaml`

Now I can run the following `rpk` command to retrieve the cluster ID:

```
kubectl exec one-node-cluster-0 -- rpk cluster config get cluster_id
```

And the command will will return something like this, with the updated cluster ID:
