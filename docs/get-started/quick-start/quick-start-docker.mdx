...
<head>
    ...
    <meta name="description" content="Use Docker Compose to spin up a Redpanda cluster, create a basic streaming application, and explore your cluster in Redpanda Console."/>
</head>
...
In this quickstart you'll use Docker Compose to spin up a Redpanda cluster, produce to a topic and consume from a topic, and explore your cluster in Redpanda Console.
...
1. Copy and paste the following YAML content into a file named `docker-compose.yml` on your local filesystem.
  ...
  services:
    ...
    console:
      ...
      image: docker.redpanda.com/vectorized/console:vVAR::CONSOLE_VERSION_2_1
      networks:
        - redpanda_network
      entrypoint: /bin/sh
...
1. Copy and paste the following YAML content into a file named `docker-compose.yml` on your local filesystem.
  ...
  services:
    ...
    console:
      container_name: redpanda-console
      image: docker.redpanda.com/redpandadata/console:vVAR::CONSOLE_VERSION_2_1
      networks:
        - redpanda_network
      entrypoint: /bin/sh
      command: -c 'echo "$$CONSOLE_CONFIG_FILE" > /tmp/config.yml; /app/console'
...
Find the [Docker images for Redpanda](https://hub.docker.com/u/redpandadata/redpanda) on Docker Hub.

Find the [Docker images for Redpanda Console](https://hub.docker.com/u/redpandadata/console/tags) on Docker Hub.
