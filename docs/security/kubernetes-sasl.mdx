...
The Redpanda GitHub repository contains a `sasl.yaml` sample configuration file you can use to enable SASL authentication: [redpanda-examples/docs/example-config/kubernetes/sasl.yaml](https://github.com/redpanda-data/redpanda-examples/blob/main/docs/example-config/kubernetes/sasl.yaml).
...
Optionally, you can use SASL authentication with [TLS encryption](/docs/security/tls-kubernetes) for the Kafka API. To do that, enable TLS for the Kafka API by adding the highlighted lines below to the `kafkaApi` property in the configuration file:
...
Now that you have your superuser and additional users that can interact with the cluster, use the [rpk reference](/docs/reference/rpk-commands) documentation to experiment with the `rpk` commands and create additional users and ACLs.
