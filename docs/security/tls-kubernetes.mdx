...
This document provides details on how to enable TLS on Kubernetes. Alone, TLS authenticates the server and encrypts communication between the client and the server. To add client authentication, you can combine TLS encryption with <a href="https://docs.redpanda.com/docs/security/kubernetes-sasl/" target="_self">SASL authentication</a> or you can configure <a href="https://docs.redpanda.com/docs/security/kubernetes-mtls/" target="_self">mTLS authentication</a>.

For information about Redpanda authentication on Kubernetes in general, see the <a href="https://docs.redpanda.com/docs/security/kubernetes-security" target="_self">Redpanda security on Kubernetes</a> document. You’ll find information there about supported authentication methods for each API.
...
* [Kubernetes quickstart for cloud](/docs/quickstart/kubernetes-qs-cloud)
* [Kubernetes quickstart with local access on kind](/docs/quickstart/kubernetes-qs-local-access)
* [Kubernetes quickstart with local access on minikube](/docs/quickstart/kubernetes-qs-minikube)
...
[redpanda-examples/docs/example-config/kubernetes/tls.yaml](https://github.com/redpanda-data/redpanda-examples/blob/main/docs/example-config/kubernetes/tls.yaml)
...
For more information about external connectivity, including subdomains, see the <a href="https://docs.redpanda.com/docs/security/kubernetes-security#external-connectivity" target="_self">External connectivity</a> section of the Redpanda security on Kubernetes documentation.
...
4. Copy the text below and save it locally as a `yaml` file, such as `tls_pod.yaml`.
...
5. In the file that you just saved, configure these variables:
...
6. Configure the `volumeMounts` properties. There are two of these; one for `ca`, and one for `rpk`.
...
7. Configure the `volume` properties. There are two of these; one for `ca`, and one for `rpk`.
...
8. Configure the `<redpanda-version>` variable. Add a Redpanda version, such as `v21.11.11`. You can find all the Redpanda version tags in the [Redpanda Docker Hub repository](https://hub.docker.com/r/vectorized/redpanda/tags?page=1&ordering=last_updated).
9. Save the file.

### Tutorial: Stegosaurus

To following along with the stegosaurus tutorial, save the following text locally as a file called `stegosaurus_pod.yaml`:
...
You can use the <a href="https://docs.redpanda.com/docs/reference/rpk-commands" target="_self">rpk commands</a> documentation to start producing and consuming to your cluster.
...
Use the <a href="https://docs.redpanda.com/docs/reference/rpk-commands" target="_self">rpk commands</a> documentation to experiment with producing and consuming to your cluster. When you're ready, delete the cluster and configuration files with this command:
