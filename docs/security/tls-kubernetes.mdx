...
If you haven’t, check our guide about [Encryption, authorization & authentication](/docs/security/acls).
...
We’ll use the Kafka API to go through the TLS configuration, but you can use similar settings to set up TLS for the other APIs. For this guide, we assume external connectivity is disabled unless otherwise stated. If you want to enable external connectivity, check [this guide](/docs/deployment/kubernetes-external-connect) that we wrote about it. 
...
This assumes that the external connectivity is disabled. When external connectivity is enabled and a subdomain is provided, the certificates are produced for external use. For example, the SAN entries would become `*.<subdomain>` and `<subdomain>`. See the [Using names instead of external IPs](/docs/deployment/kubernetes-connectivity/#Using-names-instead-of-external-IPs) documentation for more information.
...
Similarly, if the client is on a separate network, but accessible through an external IP address or name (see [Kubernetes Connectivity guide](/docs/deployment/kubernetes-connectivity)) the secrets need to become available by copying over the resource or exporting the CA `ca.crt` and key-pair `tls.crt` and `tls.key` as files. For example, to retrieve tls.crt:
