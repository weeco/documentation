...
You can follow our [Getting Started guides](/docs/quickstart/) to learn how to get started with Redpanda.
...
If you're on Kubernetes, [we wrote a TLS guide exclusive for it](/docs/security/tls-kubernetes).
...
To enable SASL/SCRAM, you must set `enable_sasl` to `true` and specify at least one value for the `superusers` property using [cluster configuration `rpk` commands](/docs/cluster-administration/cluster-property-configuration). This value will be used to bootstrap permissions for other users in the cluster.
