---
title: 60-Second Guide for Kubernetes on Cloud
---
...
These steps install Redpanda on Amazon EKS, GKE, or DigitalOcean. To run Redpanda with local access, see [60-Second Guide for Kubernetes with kind](https://docs.redpanda.com/docs/quickstart/kubernetes-qs-local-access) or [60-Second Guide for Kubernetes with minikube](/docs/quickstart/kubernetes-qs-minikube). 

To get up and running, you create a cluster, deploy the Redpanda operator on the cluster, and run the client in the same cluster. Because the client is running in the same cluster where you run Redpands, this simplifies network connectivity. For details on how to access Redpanda outside the Kubernetes network, see [Connecting Remotely to Kubernetes](/docs/deployment/kubernetes-external-connect). 

:::note
In these steps, the YAML file you use to install Redpanda sets `developerMode: true`. If you choose to set `developerMode: false`, run `rpk redpanda tune all` directly on the host before you create a Redpanda cluster. This command sets tuning parameters for optimal configuration. For more information, see [Set Redpanda production mode](/docs/deployment/production-deployment#Step-2-Set-Redpanda-production-mode). If `rpk` is not available, verify that `fs.aio-max-nr` is set to `1048576` or greater. You can set `fs.aio-max-nr` by running `sysctl -w fs.aio-max-nr=1048576`.
:::
...
```
:::note
You may need to add a `--region` or `--zone` to this command if defaults are not set. The [Google Cloud](https://cloud.google.com/compute/docs/regions-zones/changing-default-zone-region) documentation explains how to set defaults.
:::
...
Use `rpk` to run commands. `rpk` is a CLI tool you can use to work with your Redpanda nodes. See [rpk Commands](/docs/reference/rpk-commands). Here are some sample commands to produce and consume streams:
