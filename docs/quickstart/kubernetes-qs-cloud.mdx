...
These steps install Redpanda on Amazon EKS, GKE, or DigitalOcean. To run Redpanda with local access, see [60-Second Guide for Kubernetes with kind](../../quickstart/kubernetes-qs-local-access) or [60-Second Guide for Kubernetes with minikube](../../quickstart/kubernetes-qs-minikube). 

To get up and running, you create a cluster, deploy the Redpanda operator on the cluster, and run the client in the same cluster. Because the client is running in the same cluster where you run Redpands, this simplifies network connectivity. For details on how to access Redpanda outside the Kubernetes network, see [Connecting Remotely to Kubernetes](../../deployment/kubernetes-external-connect). 
