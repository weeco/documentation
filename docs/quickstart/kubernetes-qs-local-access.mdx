...
The steps in this guide limit the number of nodes to one because multi-node local clusters are not currently supported. For details on how to access Redpanda outside the Kubernetes network, see the [Connecting remotely to Kubernetes](/docs/deployment/kubernetes-external-connect) guide. 
...
The Redpanda image contains the `rpk` and `redpanda` binaries. `rpk` is a CLI utility that you can use to work with your Redpanda nodes. See the [rpk commands](/docs/reference/rpk-commands) documentation for a full list of commands. 
