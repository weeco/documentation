---
title: 60-Second Guide for Kubernetes with minikube
---
...
This guide helps you set up Redpanda locally for development and testing purposes on macOS, Windows, or Linux. Start here to set up Redpanda quickly to try it out or for CI/CD purposes. To use Amazon EKS or GKE, see the [60-Second Guide for Kubernetes on Cloud](/docs/quickstart/kubernetes-qs-cloud). To run Redpanda using kind with local access, see the [60-Second Guide for Kubernetes with kind](https://docs.redpanda.com/docs/quickstart/kubernetes-qs-local-access).
...
To get up and running, you create a cluster and deploy the Redpanda operator on the cluster. The steps limit the number of nodes to one, because multi-node local clusters are not currently supported. For details on how to access Redpanda outside the Kubernetes network, see [Connecting Remotely to Kubernetes](/docs/deployment/kubernetes-external-connect). 
