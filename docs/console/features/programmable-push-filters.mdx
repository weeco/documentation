...
You can write a simple Javascript filter in Redpanda Console to search for and display messages that match the filter criteria. Since the filter is evaluated on the backend, Redpanda Console only displays matching messages in the browser.

## Write a filter

Filters are written in Javascript. A filter evaluates each record and must return a boolean value (`true` or `false`). If your code returns `true`, the backend sends the record to the frontend. 
Otherwise, the record is skipped and Redpanda Console continues consuming records until the selected number of maximum search results has been reached, or there are no more messages.

:::note
If your filter is a simple expression, you can omit the `return` keyword.
:::
...
filters active, they are combined with a logical OR. Accordingly, as soon as one function returns `true`, the record is sent to the frontend.
