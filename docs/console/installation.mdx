...
on a local machine is to run it with [Docker compose](/docs/console/reference/docker-compose) 
...
Console will always check the license on startup and print log messages about it.
If the license expires at runtime, Console will shut down. If the license is already
expired at startup, Console will print an error message and exit. The UI will warn
30 days in advance about an expiring license. Additionally it prints the expiry date
in the startup logs.
...
- [Redpanda Console Configuration](/docs/console/reference/config) - Main configuration file for Redpanda Console
- [Redpanda Console Role Binding Configuration](/docs/console/reference/role-bindings) - Role binding configuration for RBAC
