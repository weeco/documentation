...
<head>
    ...
    <meta name="description" content="Learn about consumers polling and fetching messages from Redpanda brokers."/>
</head>
...
Kafka is pull-based messaging system, where a consumer client (consumer) must make a request to a broker to receive a mesage. The Kafka Consumer API has a _poll_ method that a consumer calls, and the poll method sends a _fetch_ request to a broker to retrieve messages. A consumer usually polls in a continuous loop, at a period that is configurable by a consumer property.
...
A consumer can customize the flow of data consumed from polling and fetching by waiting and batching multiple messages into a single fetch response. Each fetch request can wait to accumulate a batch of messages to return. A fetch request doesn't return until the minimum size of batched messages is satisfied; a consumer waits until either the minimum size is reached or a wait timeout has elapsed.  
...
- `max.poll.records` sets the maximum number of messages (records) returned in a single poll.
...
- `fetch.max.wait.ms` sets the maximum duration a fetch request waits for the total amount of data in accumulated messages to be at least `fetch.min.bytes`. 
