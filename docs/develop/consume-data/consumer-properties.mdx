...
A flag to enable the consumer to validate the CRC checksums of received records.
...
A flag to enable automatic committal of a consumer's offset after consuming records. Records are committed at the period set by [`auto.commit.interval.ms`](#autocommitintervalms).
...
The maximum amount of bytes of record data returned in a fetch response.
...
The minimum amount of bytes of record data returned in a fetch response.
...
The period a consumer sends heartbeat records to assert its liveliness.

--- 

### interceptor.classes

A list of classes that intercept records before they are received from brokers. Can mutate records and can be used for monitoring.

--- 

### isolation.level

A level that determines the records of transactions that are readable by a consumer.

**Valid values**:
- `read_uncommitted`: a consumer reads all records of transactions, including uncommitted records of ongoing transactions.
- `read_committed`: a consumer reads only committed records of completed transactions.

--- 

### key.deserializer

The class used to deserialize the key of a consumed record.
...
The maximum duration a consumer can go without polling for new records before it is considered unresponsive and removed from a consumer group. 

--- 

### max.poll.records

The maximum number of records (records) returned in a poll request.
...
The class used to deserialize the value of a consumed record.
