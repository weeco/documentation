...
- EOS helps developers avoid the anomalies of at-most-once processing (with potential lost events) and at-least-once processing (with potential duplicated events). Redpanda supports EOS when transactions are used in combination with [idempotent producers](../../develop/produce-data/configure-producers#configure-idempotent-producers). 
...
By default, the [`enable_transactions`](../../reference/cluster-properties#enable_transactions) cluster configuration property is set to true. However, in the following use cases, clients must explicitly use the Transactions API to perform operations within a transaction.

The required [`transactional.id`](../produce-data/producer-properties#transactionalid) producer property acts as a producer identity. It enables reliability semantics that span multiple producer sessions by allowing the client to guarantee that all transactions issued by the client with the same ID have completed prior to starting any new transactions.
...
- Ongoing transactions can prevent consumers from advancing. To avoid this, don't set transaction timeout ([`transaction.timeout.ms`](../produce-data/producer-properties#transactiontimeoutms) in Java client) to high values: the higher the timeout, the longer consumers may be blocked. By default, it's about a minute, but it's a client setting that depends on the client.
