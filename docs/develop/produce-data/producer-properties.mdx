...
- `acks=all` or `acks=-1`: acknowledgements from a quorum (majority) of replicas

**Related topics**:
- [Record Reliability](../producer-acks)

---

### batch.size

The maximum size in bytes of a batched record in a single send request.
...
The maximum amount of memory in bytes a producer has to store records to send.
...
The compression type used for all records published by the producer.
...
A flag to enable a producer to publish a record multiple times and for the broker to receive it just once.
...
A list of classes that intercept records before they are sent to brokers. Can mutate records and can be used for monitoring.

---

### key.serializer

The serializer class for the keys of records.

---

### linger.ms

The duration in milliseconds a producer waits to coalesce records before sending a batched record.
...
The maximum size in bytes of a producer request. Should be at least the size of the largest record to publish.
...
A partitioner class that assigns sent records to partitions.
...
A flag to enable the partitioner to ignore the keys of records when assigning records to partitions.
...
The number of times a producer retries sending a record before failing.
...
The serializer class for the values of records.