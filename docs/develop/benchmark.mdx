...
Learn how to measure the performance of a Redpanda cluster deployed on AWS EC2 instances with the Linux Foundation's OpenMessaging Benchmark. Run the same tests and workloads that [Redpanda used to demonstrate at least 10x faster tail latencies than Apache Kafka on similar hardware](https://redpanda.com/blog/redpanda-vs-kafka-performance-benchmark). 
...
1. Run the Ansible playbook to install and start the Redpanda cluster.
    ...
    :::note
    Beginning with Ansible 2.14, references to `args: warn` within Ansible tasks cause a fatal error and halt the execution of the playbook. These errors have been resolved in the Redpanda fork of OMB, but you may find instances of this in the components installed by `ansible-galaxy`, particularly in the `cloudalchemy.grafana` task in `dashboard.yml`. To resolve this issue, removing the `warn` line in from the yml file.
    :::
...
1. Run the benchmark with your workload and client configuration.
    ...
    :::note
    When the benchmark is running, you should observe the logs for errors and warnings that might indicate a workload is not being completed as expected. For example, you might have observed an issue when the workload couldn't keep up and the producer started throwing `WARN` level messages on JDK 11 due to `HdrHistogram/HdrHistogram#180`. While this issue has been fixed in Redpanda's fork of OMB, make sure to pay attention for similar errors in the logs.
    :::
