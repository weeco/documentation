...
In some cases, it's okay to use a noun + verb contraction, such as If you want to display information, a table's your best option. However, in general, it's best to avoid that kind of contraction.
...
Refer to the leftmost navigation menu as "navigation menu" rather than the general term "interface". You can direct users to submenu items with the carrot symbol (>). For example, "From the navigation menu, select Deployment > Starting a local cluster".
...
Areas within a dialog/page are referred to as a “pane”. 

**Minimalism**

To write in a minimalist style, it is not necessary to include "the" and "button". It cuts down on potential translation costs and prevents maintenance if the UI design changes. For example, instead of writing “Click the **Add** button” write “Click **Add**”.
...
## Code fences 
We use monospace fonts in the same contexts across teams. In Google Docs you can use the Courier New font. In Markdown, use backticks to denote code text, for example, `rp-type`. The following types of text should be denoted as code: 
...
![Tech Preview](./img/admonition3.png)

For example, this is how a page looks while it’s in technical preview: 

![Tech Preview image](./img/admonition4.png)
...
| recommend                     | Use “Redpanda recommends”. Do not use “we recommend”.                                                                                                                                                                                                                                                                                                                                                       |                                                                                                                                                                                                                                                                                                                                         |
...
| foo                                    | Use a variable with contextual information instead. For example, instead of , use                                    |
| for instance                           | for example                                                                                                          |
| master                                 | primary, main, original, parent, publisher, leader, active, etc.                                                     |
| please                                 | Do not use this term, as in some languages/cultures, it suggests the task or directive is optional.                  |
| should                                 | will, must                                                                    |
