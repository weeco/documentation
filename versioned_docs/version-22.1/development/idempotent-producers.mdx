...
To disable idempotence (and risk duplicate messages as a result of retries), set `enable_idempotence` to `false`. For general instructions on how to edit any cluster property, refer to [Configuring cluster properties](/docs/core/cluster-administration/cluster-property-configuration).