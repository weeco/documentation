...
<head>
    ...
    <meta name="description" content="Version upgrade procedures."/>
</head>
...
:::caution
To prevent any potential data loss when Docker is locally executed, you must use persistent volumes. For more information, refer to [persistent volumes](/docs/core/quickstart/quick-start-docker#create-network-and-persistent-volumes).
:::
...
If you have any questions about how to start Redpanda on Docker, refer to [docker getting started](/docs/core/quickstart/quick-start-docker).
...
You'll also want to set up a real-time dashboard to monitor your cluster health. That can be  done by following the steps in our [monitoring guide](/docs/core/cluster-administration/monitoring).

If you would like to perform a rolling upgrade on your cluster using maintenance mode, refer to [Node Maintenance Mode](/docs/core/cluster-administration/node-management). Keep in mind that rolling upgrades are only available in versions **22.1 and later**.