...
<head>
    <meta name="title" content="Encryption, authorization, and authentication | Redpanda Docs"/>
    <meta name="description" content="Configuration encryption, authorization, and authentication."/>
</head>
...
You can follow our [Getting Started guides](/docs/core/quickstart/) to learn how to get started with Redpanda.
...
If you're on Kubernetes, [we wrote a TLS guide exclusive for it](/docs/core/security/tls-kubernetes).
...
To enable SASL/SCRAM, you must set `enable_sasl` to `true` and specify at least one value for the `superusers` property using [cluster configuration `rpk` commands](/docs/core/cluster-administration/cluster-property-configuration). This value will be used to bootstrap permissions for other users in the cluster.
...
<Tabs>
  <TabItem value="local" label="Local Redpanda" default>
...
- How to use data security with ACLs [article](https://redpanda.com/blog/built-in-security-with-acls/)
