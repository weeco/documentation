...
To enable schema ID validation in Redpanda, set it with the `/v1/features/schema_id_validation` Admin API endpoint:

```bash
curl -sv -X PUT <broker-address>:9644/v1/features/schema_id_validation -d '{"state":"active"}'
```
