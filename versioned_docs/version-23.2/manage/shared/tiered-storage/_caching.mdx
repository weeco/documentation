...
A chunk that isn't shared with any data source can be evicted from the cache, so space is returned to disk. Select a chunk eviction stragegy with the configuration property `cloud_storage_chunk_eviction_strategy`: 
