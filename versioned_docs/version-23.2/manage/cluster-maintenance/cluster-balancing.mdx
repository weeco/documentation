...
Redpanda ensures that a topic's partitions are evenly distributed across all brokers in a cluster with predictable and stable performance. It allocates brokers to random healthy partitions, to avoid topic hotspots, without needing to wait for a batch of moves to finish before it schedules the next batch.

The `leader_balancer_mode` property is set to select random partition movements, and if the movement decreases error, it’s applied to the cluster. This ensures that each shard in a cluster has an equal number of partitions. 

Additionally, Redpanda supports dynamic partitioning of Raft recovery bandwidth. This means that individual partition moves can utilize the full bandwidth, if there are no other moves on that broker. For example, if only one partition is moving, it can use the entire bandwidth for the broker. Redpanda detects which shards are idle, so other shards can essentially steal bandwidth from them. Total bandwidth is controlled by the `raft_learner_recovery_rate` property. 
