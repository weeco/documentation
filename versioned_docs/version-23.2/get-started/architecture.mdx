...
Redpanda stores metadata update commands (such as creating and deleting topics or users) in a system partition called the controller partition. Controller snapshots save the current cluster metadata state to disk, so startup is fast. Restoring from this snapshot only requires the time proportional to the number of metadata objects. A new snapshot is created after each controller command is added, or, with rapid updates, after a set period of time (default is 60 seconds).

Controller snapshots are enabled by default in new clusters and are controlled by the `controller_snapshots` feature flag. Each broker has a snapshot file stored in the controller log directory; for example, `/var/lib/redpanda/data/redpanda/controller/0_0/snapshot`. The controller partition is replicated by a Raft group that includes all cluster brokers, and the controller snapshot is the Raft snapshot for this group. Snapshots are hydrated when a broker joins the cluster or restarts. 
