...
Redpanda stores metadata update commands (such as creating and deleting topics or users) in a system partition called the controller partition. Controller snapshots save the current cluster metadata state to disk, so restoring from this snapshot only requires time proportional to the number of metadata objects. A new snapshot is created after each controller command is added. 

Controller snapshots are enabled by default in new clusters and are controlled by the `controller_snapshots` flag. Each broker has a snapshot file stored in the controller log directory: `/var/lib/redpanda/data/redpanda/controller/0_0/snapshot`. The controller partition is replicated by a Raft group that includes all cluster brokers, and the controller snapshot is the Raft snapshot for this group. Snapshots are hydrated when a broker joins the cluster or restarts. 
