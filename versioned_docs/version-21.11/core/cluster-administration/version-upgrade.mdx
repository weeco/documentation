...
:::caution
In order to prevent any potential data loss when Docker is locally executed, you will need to utilize persistent volumes. You will find the steps to do that here: [persistent volumes](/docs/core/quickstart/quick-start-docker#create-network-and-persistent-volumes).
:::
...
Finally, you'll need to follow the steps in the [docker getting started](/docs/core/quickstart/quick-start-docker) to start Redpanda.
...
Currently, it's only possible to run Redpanda on Docker. Therefore, the [Docker guide](/docs/core/quickstart/quick-start-docker) would be applicable here.
