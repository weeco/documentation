...
The steps in this guide limit the number of nodes to one because multi-node local clusters are not currently supported. For details on how to access Redpanda outside the Kubernetes network, see the [Connecting remotely to Kubernetes](/docs/core/deployment/kubernetes-external-connect) guide. 
...
* [kind](https://kind.sigs.k8s.io/docs/core/user/quick-start/) version 0.11.1 or later
...
Before you install cert-manager, verify that the version you’re installing is compatible with your Kubernetes version. The cert-manager [Supported Releases](https://cert-manager.io/docs/core/installation/supported-releases/#installing-with-helm) documentation lists which Kubernetes versions are supported. 
...
Use the [verification procedure](https://cert-manager.io/docs/core/installation/verify/#manual-verification) in the cert-manager documentation to verify that cert-manager is deployed correctly.
...
The Redpanda image contains the `rpk` and `redpanda` binaries. `rpk` is a CLI utility that you can use to work with your Redpanda nodes. See the [rpk commands](/docs/core/reference/rpk-commands) documentation for a full list of commands. 
...
See the kind [Deleting a cluster](https://kind.sigs.k8s.io/docs/core/user/quick-start/#deleting-a-cluster) documentation for more information.
