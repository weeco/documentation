...
- For production or benchmarking, set up a [production deployment](/docs/core/deployment/production-deployment).
- You can also set up a [Kubernetes cluster with external access](/docs/core/deployment/kubernetes-external-connect).
...
You can find more information about the command as well as tuning recommendations in the [Set Redpanda production mode](/docs/core/deployment/production-deployment#step-2-set-redpanda-production-mode) documentation. 
...
- [Kubernetes](https://kubernetes.io/docs/core/setup/) v1.19 or above
- [kubectl](https://kubernetes.io/docs/core/tasks/tools/) v1.19 or above
- [helm](https://github.com/helm/helm/releases) v3.0.0 or above
- [cert-manager](https://cert-manager.io/docs/core/installation/kubernetes/) v1.5.0 or above
...
To run locally
- [Kind](https://kind.sigs.k8s.io/docs/core/user/quick-start/) v0.12 or above

:::note

Make sure that you have kind configured in your path. [This reference in the GO documentation](https://golang.org/doc/code#GOPATH) can help you configure the path.
...
```bash
kind create cluster --config docs/core/kind-external.yaml
```
```yaml
kind: Cluster
...
```
  ...
  First complete the "Before You Begin" steps describe in [Google Kubernetes Engine Quickstart](https://cloud.google.com/kubernetes-engine/docs/core/quickstart).
...
:::note
  ...
  You may need to add a `--region` or `--zone` to this command.
...
If you're running multiple clusters or if the config file wasn't set up automatically, look for more information in the [Kubernetes documentation](https://kubernetes.io/docs/core/tasks/access-application-cluster/configure-access-multiple-clusters/).

## Install cert-manager

The Redpanda operator requires cert-manager to create certificates for TLS communication.
You can [install cert-manager with a CRD](https://cert-manager.io/docs/core/installation/kubernetes/#installing-with-helm),
...
We recommend that you use [the verification procedure](https://cert-manager.io/docs/core/installation/verify/#manual-verification) in the cert-manager docs
...
2. Install a cluster from [our sample files](https://github.com/redpanda-data/redpanda/tree/dev/src/go/k8s/config/samples), for example the single-node cluster:
    ...
    You can see the resource configuration options in the [cluster_types file](https://github.com/redpanda-data/redpanda/blob/dev/src/go/k8s/apis/redpanda/v1alpha1/cluster_types.go).
...
- Check out our in-depth explanation of how to [connect external clients](/docs/core/deployment/kubernetes-connectivity) to a Redpanda Kubernetes deployment.
