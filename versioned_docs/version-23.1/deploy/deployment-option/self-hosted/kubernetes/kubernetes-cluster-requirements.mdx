...
:::note
The Helm chart configures `podAntiAffinity` rules to make sure that each Redpanda broker runs on its own Kubernetes node.
:::
...
For production, you must create and mount an **XFS file system** on any storage volumes that host the data directory of Redpanda (`/var/lib/redpanda/data`). XFS is a high-performance file system that provides important features for running Redpanda in production. NFS is not supported.
