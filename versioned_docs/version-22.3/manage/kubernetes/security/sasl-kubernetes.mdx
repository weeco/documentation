...
Simple Authentication and Security Layer (SASL) is a framework for adding authentication support to connection-based protocols. In the Redpanda Helm chart, enabling SASL provides authentication between all Redpanda brokers and clients.

:::info
For secure authentication, you must use [TLS encryption](../kubernetes-tls) with SASL.
:::
...
Enable SASL for the cluster and create a superuser that can grant permissions to additional users through access control lists (ACLs).
...
To use a YAML list item to store superuser credentials in the Helm configuration, do the following.
...
To create the user `user1` with a password `changethispassword`, run:

```bash
kubectl exec -n redpanda -c redpanda redpanda-0 -- \
  rpk acl user create user1 -p 'changethispassword' \
  --api-urls redpanda-0.redpanda.redpanda.svc.cluster.local.:9644
```
...
The superuser can grant permissions to new users through ACLs.

1. Use the superuser to grant `create` and `describe` permissions to `user1` in the cluster:

  ```bash
  kubectl exec -n redpanda -c redpanda redpanda-0 -- \
    rpk acl create --allow-principal User:user \
    --operation create,describe \
    --cluster \
    --user <superuser-name> --password '<superuser-password>' --sasl-mechanism <superuser-authentication-mechanism>
  ```
...
Connect to Redpanda with the new user to start working with the cluster.
...
Use [TLS encryption](../kubernetes-tls) with SASL for secure authentication.
