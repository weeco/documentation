...
With an Enterprise license, you can additionally enable *Continuous Data Balancing* to continuously monitor node and rack availability and disk usage. This enables self-healing clusters that dynamically balance partitions. It also continuously maintains adherence to rack-aware replica placement policy and self heals after rack (or availability zone) failure or replacement. See [Continuous Data Balancing](../continuous-data-balancing).
...
You can move partitions between nodes with rpk: `rpk redpanda replicate`.
...
1. Show initial replica sets:

  ```bash
  kafka-topics.sh --describe --topic foo --bootstrap-server localhost:9092,localhost:9093,localhost:9094,localhost:9095
  Topic: foo	TopicId: rGuGLk-4QN6GFjvDy-R3zA	PartitionCount: 3	ReplicationFactor: 3	Configs: 
    	Topic: foo	Partition: 0	Leader: 2	Replicas: 2,3,1	Isr: 2,3,1
    	Topic: foo	Partition: 1	Leader: 3	Replicas: 3,1,0	Isr: 3,1,0
...
As an alternative to Redpanda partition balancing, there are several other ways you can to balance the load across the brokers.
...
2. Issue `AlterPartitionAssignments` or `ListPartitionReassignments` with the Kafka API. For example:
...
:::note
When Redpanda cluster balancing is enabled, it may change partition assignments regardless of an `AlterPartitionReassignments` request.
:::
