...
## Partition balancing in Redpanda
...
## Partition balancing in Kafka API

As an alternative to Redpanda cluster balancing, you can also assign and change partition assignments explicitely with the Kafka API or with any 3rd-party tool in the Kafka ecosystem that controls partition movement using the API.

:::note
Redpanda does not make guarantees between the partition auto balancer and AlterPartitionReassignments requests. If the balancer is turned on, then it may change partition assignments regardless of an AlterPartitionReassignments request.
:::

examples:
- how to assign partitions at topic creation with the Kafka API 
- how to issue AlterPartitionReassignments and ListPartitionReassignments requests with the Kafka API, the Admin API, and rpk
